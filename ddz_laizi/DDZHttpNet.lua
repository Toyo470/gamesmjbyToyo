...
local DDZHttpNet  = class("DDZHttpNet")


--创建http GET请求
function DDZHttpNet:create(strUrl,content,callback)
...
--请求游戏人数
function DDZHttpNet:loadGame(uid,gameid)
	local function backLand( event )
		...
		if  SCENENOW["name"] == "ddz_laizi.ddzScene" then
...
--请求自由场赛场人数
function DDZHttpNet:FreeloadBattles(battleid)
	local function backLand( event )
		local request = event.request
...
--请求比赛场赛场人数
function DDZHttpNet:MatchloadBattles(battleid)
	local function backLand( event )
		local request = event.request
	    ...
	    for i, v in ipairs(gameList["data"]["roomList"]) do
	        if v["level"] ~= nil and v["fee"] ~= nil then
	            ...
	            require("ddz_laizi.ddzSettings"):addMatchRank(v["level"],tbRank)
	        end
	    end
...
--gameid：游戏ID
--uid：用户ID
function DDZHttpNet:applyGameAwardPool()
...
--请求比赛奖励配置
function DDZHttpNet:requestIncentive( matchLevel )
	-- body
    ...
    local function reqCallback(event)
        ...
        require("ddz_laizi.MatchSetting"):setIncentive(data)
