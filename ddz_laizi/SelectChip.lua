...
function SelectChip:ctor()
    ...
    local s = cc.CSLoader:createNode("ddz/csb/BetSelect.csb"):addTo(self)
    self._scene = s
    ...
    local function touchButtonEvent(sender, event)
        ...
        if event == TOUCH_EVENT_ENDED then
            ...
            if sender == btn_lobby then--返回赛场选择
                ...
                display_scene("ddz_laizi.ddzScene",1)
            end
...
function SelectChip:addBattleField(index,chip,counts,level)
    ...
    local strFile = "ddz/BetSelect/classical_bt_"..tostring(index)..".png"
    ...
    --房间名
    strFile = "ddz/BetSelect/classical_text_"..tostring(index)..".png"
    local sp = cc.Sprite:create(strFile)
    button:addChild(sp)
    sp:setPosition(183, 135)

    strFile = "ddz/BetSelect/classical_box_"..tostring(index)..".png"
    sp = cc.Sprite:create(strFile)
    button:addChild(sp)
    sp:setPosition(183, 87)

    strFile = "ddz/BetSelect/ziyou_people.png"
    sp = cc.Sprite:create(strFile)
    button:addChild(sp)
    sp:setPosition(47, 43)

    strFile = "ddz/BetSelect/ziyou_gold.png"
    sp = cc.Sprite:create(strFile)
    button:addChild(sp)
    sp:setPosition(208, 43)

    if USER_INFO["landlord_section"]["ddz_laizi"] then
        local data = USER_INFO["landlord_section"]["ddz_laizi"]
        local str = ""
    ...
    local function touchBattle(sender, event)
        ...
        if event == TOUCH_EVENT_ENDED then
            ...
            if sender == button then
                ...
                -- if USER_INFO["gold"] > tbBattles[button]["chip"]*3 then
                local ret = require("hall.hall_data"):checkEnter("ddz_laizi",tbBattles[button]["level"])
                if ret == 1 then--少于低分
...
function SelectChip:addMatchBattle(index,chip,cost4match,counts,playerLevel,matchLevel,matchId,mode)
    ...
    -- local lbTitle = cc.Label:createWithTTF("1000-1万","res/fonts/fzcy.ttf",24)
    ...
    local function touchBattle(sender, event)
        ...
        if event == TOUCH_EVENT_ENDED then
            ...
            if USER_INFO["gold"] > tbMatchFee[button] then
                require("ddz.ddzSettings"):setMatchId(matchId)
                require("ddz.ddzSettings"):setMatchLevel(matchLevel)
                require("ddz.MatchSetting"):showMatchSignup(true,0,playerLevel,tbMatchFee[button],"ddz",matchLevel)
                -- require("hall.GameData"):setPlayerRoute("ddz","match",tbMatchFee[button],0)
            else
...
function SelectChip:onEnter()
    ...
    if mode == "free" then
        HttpNet:FreeloadBattles(101)
    elseif mode == "match" then
        HttpNet:MatchloadBattles(101)
    end

    require("ddz_laizi.ddzSettings"):reset()
    bMatchJoin = false
    ...
    if USER_INFO["match_fee"] then
        ...
        if USER_INFO["match_fee"] > 0 then
            require("ddz_laizi.ddzServer"):CLI_SEND_JOIN_MATCH(require("ddz_laizi.ddzSettings"):getMatchLevel())
        end
    else
...
function SelectChip:onNetMatchJoin(pack)
    ...
    require("ddz_laizi.ddzSettings"):setMatchId(tid)
    print("MatchID id:%d",require("ddz_laizi.ddzSettings"):getMatchId())
...
function SelectChip:onNetMatchLoginFailed(pack)
    ...
    require("ddz_laizi.MatchSetting"):setJoinMatch(false)
end
--成功进入比赛
function SelectChip:onNetMatchLoginOk(pack)
    ...
    if USER_INFO["match_fee"] then
        if USER_INFO["match_fee"] > 0 then
            bMatchJoin = true
            require("ddz_laizi.MatchSetting"):showMatchSignup(true,pack["MatchUserCount"],pack["totalCount"],pack["Costformatch"],"ddz_laizi",require("ddz_laizi.ddzSettings"):getMatchLevel())
        end
    end

    require("ddz_laizi.MatchSetting"):setJoinMatch(true)
...
function SelectChip:onNetMatchOtherLogin(pack)
    --增加一个等待比赛的页面
    require("ddz_laizi.MatchSetting"):joinMatch(pack["Usercount"],pack["Totalcount"])
end
--用户退出比赛
