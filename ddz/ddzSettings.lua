...
--组局排行榜
function ddzSettings:showRanking(playerlist,game_amount)
    -- body
...
--组局，添加玩家记录
function ddzSettings:addGroupRecord(nick,buyChip,chip)
    -- body
...
--显示一局结算的结算界面
function ddzSettings:showGroupResult(groupRanking)
...
local group_state = -1
function ddzSettings:setGroupState(state)
    group_state = state
    print("setGroupState",group_state)
end

function ddzSettings:getGroupState(state)
...
-- 设置玩家ip地址
local user_ip = {}
function ddzSettings:setUserIP(uid,ip)
    if uid ~= 0 and ip then
        user_ip[uid] = ip
    end
end
function ddzSettings:getUserIP(uid)
    if user_ip == nil then
        return ""
    end
    return user_ip[uid]
