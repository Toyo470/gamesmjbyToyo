    ...
    local function touchButtonEvent(sender, event)
        ...
        if event == TOUCH_EVENT_ENDED then
            ...
            if sender == btn_fast_game then
                ...
                FastStartGame:init(USER_INFO["gold"],"ddz",1)
            end
            --帮助
...
function SelectChip:addBattleField(index,chip,counts,level)
    ...
    if USER_INFO["landlord_section"]["ddz"] then
        local data = USER_INFO["landlord_section"]["ddz"]
        local str = ""
    ...
    local function touchBattle(sender, event)
        ...
        if event == TOUCH_EVENT_ENDED then
            ...
            if sender == button then
                ...
                local ret = require("hall.hall_data"):checkEnter("ddz",tbBattles[button]["level"])
                if ret == 1 then--少于低分
...
function SelectChip:onEnter()
    ...
    if USER_INFO["match_fee"] then
        ...
        if USER_INFO["match_fee"] > 0 then
            require("ddz.ddzServer"):CLI_SEND_JOIN_MATCH(require("ddz.ddzSettings"):getMatchLevel())
        end
    else
...
function SelectChip:onNetMatchJoin(pack)
    ...
    require("ddz.ddzSettings"):setMatchId(tid)
    print("MatchID id:%d",require("ddz.ddzSettings"):getMatchId())
...
function SelectChip:onNetMatchLoginOk(pack)
    ...
    if USER_INFO["match_fee"] then
        if USER_INFO["match_fee"] > 0 then
            bMatchJoin = true
            require("ddz.MatchSetting"):showMatchSignup(true,pack["MatchUserCount"],pack["totalCount"],pack["Costformatch"],"ddz",require("ddz.ddzSettings"):getMatchLevel())
        end
    end

    require("ddz.MatchSetting"):setJoinMatch(true)
...
function SelectChip:onNetMatchOtherLogin(pack)
    --增加一个等待比赛的页面
    require("ddz.MatchSetting"):joinMatch(pack["Usercount"],pack["Totalcount"])
end
--用户退出比赛
