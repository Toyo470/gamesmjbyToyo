...
function ZZ_Handle:ctor()
  ...
  local  func_ = {
    --登陆房间返回
      [PROTOCOL.SVR_GAME_START]      = {handler(self, ZZ_Handle.SVR_GAME_START)},
    --登陆房间返回
      ...
      --服务器返回组局收到的信息
      [PROTOCOL.SERVER_CMD_FORWARD_MESSAGE] = {handler(self, ZZ_Handle.SERVER_CMD_FORWARD_MESSAGE)},
      [PROTOCOL.SERVER_CMD_MESSAGE] = {handler(self, ZZ_Handle.SERVER_CMD_MESSAGE)},
...
-- 游戏开始
function ZZ_Handle:SVR_GAME_START( pack )
  -- body
...
local time1= os.time()
  ...
  -- for _,content in pairs(pack.content) do
    ...
    if content.ifhu == 1 then
      ...
      if index~=0 then   --胡牌后不能拖动牌。  index =0 对自身的胡牌动画不影响
...
function ZZ_Handle:SVR_HUPAI_BROADCAST(pack)
 ...
 -- local layout_object = nil
  ...
  for i ,content in pairs(pack.content) do
    ...
    -- end
    --if handpeng then
      require("zz_majiang.pengresult"):show_hu_result(index)
    --end
...
function ZZ_Handle:SVR_PLAYER_USER_BROADCAST(pack)
  ...
  ZZ_Handle:showTimer(pack.uid,8)
  ...
  if pack.uid == tonumber(UID) then
    ...
    if result['p'] then
      ...
      if zzroom.manager:remove_hand_card(0, tonumber(pack["card"]),2) then
    ...
    if result['g'] or result['pg'] then
      ...
      require("zz_majiang.pengresult"):show_handle_result(index,result)
    end 
...
function ZZ_Handle:SVR_OWN_CATCH_BROADCAST(pack)
  ...
  ZZ_Handle:showTimer(tonumber(UID),8)
...
function ZZ_Handle:SVR_SEND_MAJIANG_BROADCAST(pack)
  ...
  self:showHandlesView(pack.handle,pack.card)  --处理碰杠胡的动作

  local index = zzroom.manager:get_other_index(pack.uid)  --获取出牌用户的座位id

  layout_object:remove_zhua_card()  --删除抓牌
  ...
  if index == 0 then
    if zzroom.manager:remove_hand_card(index,pack.card,1) then  -- 移除要出的牌
      local account_object = account.get_player_account()
      local sex = account_object:get_account_sex()  -- 获取用户性别
      sex =sex or 2
      music.manager:playEffectCard(sex,pack.card)  --播放出牌声音
      layout_object:drawHandCard(index)
    end


  else
    if zzroom.manager:remove_hand_card(index,0,1) then
      --播放出牌声音
      local user_info = zzroom.manager:get_user_data(pack.uid)
      local user_info_info = json.decode(user_info.user_info)
      user_info_info = user_info_info or {}
...
function ZZ_Handle:SVR_REGET_ROOM(pack)
  ...
  if not bm.Room.UserInfo then
    bm.Room.UserInfo = {}
  end
  bm.Room.UserInfo[UID] = {}
  bm.Room.UserInfo[UID]["nick"] = USER_INFO["nick"]
  if pack.nPlayerCount > 0  then
    for i,v in pairs(pack.users_info) do
      bm.Room.UserInfo[v.UserId] = {}
      bm.Room.UserInfo[v.UserId]["user_info"] = v.m_strUserInfo
    end
  end
  ...
  gold = gold - USER_INFO["group_chip"]
  local account_object = account.get_player_account()
  ...
  local layout_object = layout.manager:get_layout_object("room_base")
  if layout_object ~= nil then
    layout_object:set_left_card_num(tostring(pack["card_less"] - 4))
  end
  ...
  if pack.chupai_seatid == pack.seat_id then --到我出牌
    ...
    if rec_chupai_uid ~= nil then
      ZZ_Handle:showTimer(rec_chupai_uid,8)
    end
  end
...
--  -- layout_object:showZhuang(0)
  ...
  if user_mount > 0  then
    for i,v in pairs(pack.users_info) do
      ...
      bm.Room.UserInfo[v.uid]["user_info"] = v.user_info
    end
  end
...
function ZZ_Handle:SVR_LOGIN_ROOM_BROADCAST(pack)
 ...
 -- dump(pack)
  ...
  bm.Room.UserInfo[pack.uid] = {}
  bm.Room.UserInfo[pack.uid]["user_info"] = pack.user_info
end
...
function ZZ_Handle:SVR_SEND_USER_CARD(pack)
  ...
  local function CallFucnCallback2()
      ...
      for i=0,3 do
        if i == 0 then
          self:showPlayerCards(player_card_object,i,pack.card) --第一手牌
        else
...
--显示玩家的牌
function ZZ_Handle:showPlayerCards(layout_object, index,card )
  if card then
    layout_object:drawHandCard(index,true,card)
  else
    layout_object:drawHandCard(index)
  end
...
--服务器返回组局收到的信息
function ZZ_Handle:SERVER_CMD_FORWARD_MESSAGE( pack )
  dump(pack,"-------距离数据---------")   
...
function ZZ_Handle:SERVER_CMD_MESSAGE( pack )
