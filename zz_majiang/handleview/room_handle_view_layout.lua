...
-- local Template = require("zz_majiang.zzroom.room_card_template.room_card_Template")
mrequire("handleview.room_handle_view_template")
-- mrequire("zzroom.room_card_template.room_card_Template")
mrequire("layout")
...
function HandleViewLayout:click_room_handle_view_hu_event()
	...
	self:requestHandle(result)
  if layout~=nil then
    local room_card_object= layout.manager:get_layout_object("room_card")   --胡牌隐藏听牌内容
    if room_card_object~=nil then
      room_card_object:hide_ting_base(false)
    end
