...
function FileUtils:getStringFromFile(filepath)
	...
	local test_filepath = writepath .. "zz_majiang2/"..filepath
	local filedata = cc.FileUtils:getInstance():getStringFromFile(test_filepath)

	if filedata == "" then
 		filedata = cc.FileUtils:getInstance():getStringFromFile("zz_majiang2/"..filepath)
	end
...
function FileUtils:isFileExist(test_path)
	...
	local ret = false
	local writepath = cc.FileUtils:getInstance():getWritablePath()
	local w_test_path = writepath .. "zz_majiang2/"..test_path

	ret = cc.FileUtils:getInstance():isFileExist(w_test_path)
	if ret == false then
		ret = cc.FileUtils:getInstance():isFileExist("zz_majiang2/"..test_path)
	end
	return ret
end
