...
local PROTOCOL = import("zz_majiang2.gd_Protocol")
  ...
  if applyId == uid then
    ...
    if isMyAgree == 1 then
      ...
      tips.show_tips_dissove("提示", showMsg,function()
        -- body
        zz_majiang2Server:c2sdissove_room(1)
      end,
      function()
        -- body
        zz_majiang2Server:c2sdissove_room(0)
      end,true)
    end
...
function gd_Handle:SVR_GET_ROOM_OK(pack_data)
  ...
  local pack = bm.server:createPacketBuilder(PROTOCOL.CLI_LOGIN_ROOM_GROUP)
      ...
      :setParameter("nUserId", tonumber(tonumber(UID)))
...
function gd_Handle:SVR_LOGIN_ROOM(pack)
 ...
 -- dump(group_time,"group_time")
  ...
  zz_majiang2Server:CLI_READY_GAME()
