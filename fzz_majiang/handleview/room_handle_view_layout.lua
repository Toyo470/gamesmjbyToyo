--碰杠胡等操作
local MajiangroomServer = require("fzz_majiang.MajiangroomServer")
...
function HandleViewLayout:_do_after_init()
  ...
  self.room_handle_view_change:setVisible(false)
end
...
--换牌
function HandleViewLayout:click_room_handle_view_change_event()
  local result = self.room_handle_view_change.result
  print("room_handle_view_change------result------------",result)
  self:requestHandle(result)
end
...
function HandleViewLayout:reset_state(result,ming_or_an,card)
  ...
  if result['ch'] then
    has   = 1
    self.room_handle_view_change:setVisible(true)
    self.room_handle_view_change.result = result['ch']
    self.card_value = card
    change_flag = 1
  end
  ...
  if has  == 1 then
    ...
    if change_flag == 1 then
      posX = posX - width
      self.room_handle_view_change:setPositionX(posX)
    end
