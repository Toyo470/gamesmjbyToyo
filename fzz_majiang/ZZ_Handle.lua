...
local zz_majiangServer = require("fzz_majiang.zz_majiangServer")
...
local timer_outcard = 10
local timer_operate = 10
...
function ZZ_Handle:ctor()
  ...
  local  func_ = {
    --登陆房间返回
      [PROTOCOL.SVR_LOGIN_ROOM]      = {handler(self, ZZ_Handle.SVR_LOGIN_ROOM)},
      --用户自己退出成功
      [PROTOCOL.SVR_QUICK_SUC] = {handler(self, ZZ_Handle.SVR_QUICK_SUC)},
      --广播用户准备
      [PROTOCOL.SVR_USER_READY_BROADCAST] = {handler(self, ZZ_Handle.SVR_USER_READY_BROADCAST)},
      --登陆房间广播
      [PROTOCOL.SVR_LOGIN_ROOM_BROADCAST] = {handler(self, ZZ_Handle.SVR_LOGIN_ROOM_BROADCAST)},
      --广播玩家退出返回
      [PROTOCOL.SVR_QUIT_ROOM] = {handler(self, ZZ_Handle.SVR_QUIT_ROOM)},
      --发牌
      [PROTOCOL.SVR_SEND_USER_CARD] = {handler(self, ZZ_Handle.SVR_SEND_USER_CARD)},
      --当前抓牌用户广播
      [PROTOCOL.SVR_PLAYING_UID_BROADCAST] = {handler(self, ZZ_Handle.SVR_PLAYING_UID_BROADCAST)},
      --广播用户出牌
      [PROTOCOL.SVR_SEND_MAJIANG_BROADCAST] = {handler(self, ZZ_Handle.SVR_SEND_MAJIANG_BROADCAST)},
      --svr通知我抓的牌
      [PROTOCOL.SVR_OWN_CATCH_BROADCAST] = {handler(self, ZZ_Handle.SVR_OWN_CATCH_BROADCAST)},
      --广播用户进行了什么操作
      [PROTOCOL.SVR_PLAYER_USER_BROADCAST] = {handler(self, ZZ_Handle.SVR_PLAYER_USER_BROADCAST)},
      --广播胡
      [PROTOCOL.SVR_HUPAI_BROADCAST]       = {handler(self, ZZ_Handle.SVR_HUPAI_BROADCAST)}, 
      --结算
      [PROTOCOL.SVR_ENDDING_BROADCAST] = {handler(self, ZZ_Handle.SVR_ENDDING_BROADCAST)}, 
      --请求托管
      [PROTOCOL.SVR_ROBOT] = {handler(self, ZZ_Handle.SVR_ROBOT)}, 
      --获取房间id结果
      [PROTOCOL.SVR_GET_ROOM_OK]     = {handler(self, ZZ_Handle.SVR_GET_ROOM_OK)},
        --登陆错误
      [PROTOCOL.SVR_ERROR]      = {handler(self, ZZ_Handle.SVR_ERROR)},
      --用户重新登录普通房间的消息返回（4105(10进制s)）
      [PROTOCOL.SVR_REGET_ROOM]      = {handler(self, ZZ_Handle.SVR_REGET_ROOM)},--重登
      --服务器告知客户端可以进行的操作
      [PROTOCOL.SVR_NORMAL_OPERATE]      = {handler(self, ZZ_Handle.SVR_NORMAL_OPERATE)},--广播可以进行的操作
      --广播刮风下雨（返回）杠
      --[PROTOCOL.SVR_GUFENG_XIAYU]      = {handler(self, ZZ_Handle.SVR_GUFENG_XIAYU)},
      --组局时长
      [PROTOCOL.SVR_GROUP_TIME]     = {handler(self, ZZ_Handle.SVR_GROUP_TIME)},
      --组局排行榜
      [PROTOCOL.SVR_GROUP_BILLBOARD]     = {handler(self, ZZ_Handle.SVR_GROUP_BILLBOARD)},

      [PROTOCOL.SVR_ENTER_PRIVATE_ROOM] = {handler(self, ZZ_Handle.SVR_ENTER_PRIVATE_ROOM)},


      --解散房间相关
      --没有此房间，解散房间失败  0x908
      [PROTOCOL.G2H_CMD_DISSOLVE_FAILED] = {handler(self, ZZ_Handle.G2H_CMD_DISSOLVE_FAILED)},
      --广播当前组局解散情况
      [PROTOCOL.G2H_CMD_REFRESH_DISSOLVE_LIST] = {handler(self, ZZ_Handle.G2H_CMD_REFRESH_DISSOLVE_LIST)},
      --广播桌子用户请求解散组局
      [PROTOCOL.SERVER_BROADCAST_REQUEST_DISSOLVE_GROUP] = {handler(self, ZZ_Handle.SERVER_BROADCAST_REQUEST_DISSOLVE_GROUP)},
      --广播桌子用户成功解散组局
      [PROTOCOL.SERVER_BROADCAST_DISSOLVE_GROUP] = {handler(self, ZZ_Handle.SERVER_BROADCAST_DISSOLVE_GROUP)},
      --广播桌子用户解散组局 ，解散组局失败
      [PROTOCOL.SERVER_BROADCAST_FORBIT_DISSOLVE_GROUP] = {handler(self, ZZ_Handle.SERVER_BROADCAST_FORBIT_DISSOLVE_GROUP)},

      [PROTOCOL.BROADCAST_USER_IP] = {handler(self, ZZ_Handle.BROADCAST_USER_IP)},
      [PROTOCOL.SVR_MSG_FACE] = {handler(self, ZZ_Handle.SVR_MSG_FACE)},
      -- 更新玩家分数
      [PROTOCOL.G2C_UPDATE_PLAYER_CHIP] = {handler(self, ZZ_Handle.G2C_UPDATE_PLAYER_CHIP)},
      --出牌错误返回
      [PROTOCOL.SVR_CHUPAI_ERROR] = {handler(self, ZZ_Handle.SVR_CHUPAI_ERROR)}, 
...
--没有此房间，解散房间失败  0x908
function ZZ_Handle:G2H_CMD_DISSOLVE_FAILED( pack )
 ...
 --广播当前组局解散情况
function ZZ_Handle:G2H_CMD_REFRESH_DISSOLVE_LIST( pack )
...
function ZZ_Handle:SERVER_BROADCAST_REQUEST_DISSOLVE_GROUP( pack )
  dump(pack, "-----广播桌子用户请求解散组局-----")
end

function ZZ_Handle:SERVER_BROADCAST_DISSOLVE_GROUP( pack )
...
function ZZ_Handle:SERVER_BROADCAST_FORBIT_DISSOLVE_GROUP( pack )
...
function ZZ_Handle:SVR_ENTER_PRIVATE_ROOM( pack )
  -- if layout == nil then
  --   return
  -- end
...
function ZZ_Handle:SVR_ERROR(pack)
  print("============================SVR_ERROR===============================")
  dump(pack)
...
--广播用户托管
function ZZ_Handle:SVR_ROBOT(pack)
  print("------------------------------------------------youxi tuoguan---------------------------------")
  dump(pack)
end

--结算
function ZZ_Handle:SVR_ENDDING_BROADCAST(pack)
  if layout == nil then
    return
  end
  local time1= os.time()
  ...
  local action2 = cc.Sequence:create(cc.DelayTime:create(3),cc.Show:create(),cc.CallFunc:create(function_name_result_effect))
  result_effect:runAction(action2)

  
  local niaonum =  pack.niaonum or 0
  local use_deley_time = 3  + niaonum * 0.5 + 3
  ...
  -- for _,content in pairs(pack.content) do
    ...
    if pack["count_timeout"][content_i] and pack["count_timeout"][content_i] > 0 then
      if string.len(str) > 0 then
        str = str..strSpace
      end
      str = str .. "超时x"..tostring(pack["count_timeout"][content_i])
    end
  ...
  local time2= os.time()
  print("time2--------time1--------------",time2,"----",time1,"---------time2-time1----",(time2-time1))
end


--胡牌协议
function ZZ_Handle:SVR_HUPAI_BROADCAST(pack)
 ...
 -- local layout_object = nil
  ...
  for i ,content in pairs(pack.content) do
    ...
    -- end
    --if handpeng then
      require("fzz_majiang.pengresult"):show_hu_result(index)
    --end
...
--广播用户进行了什么操作
function ZZ_Handle:SVR_PLAYER_USER_BROADCAST(pack)
  ...
  ZZ_Handle:showTimer(pack.uid,timer_outcard)

  room_card_object:remove_zhua_card()
  dump(result, "tongzhiyouhuchaozuo")
  ...
  if pack.uid == UID then
    ...
    if result['g'] or result['pg'] then
      zzroom.manager:deal_gang(0,pack.card,mingoran)
    ...
    if result['p'] then
      ...
      zzroom.manager:remove_hand_card(0,pack.card,2)
    ...
    if result['g'] or result['pg']  then
      zzroom.manager:deal_gang(index,pack.card,mingoran,result['g'])
      room_card_object:drawHandCard(index)

      music.manager:playEffectGang(sex)
      require("fzz_majiang.pengresult"):show_handle_result(index,result)
    end 
    ...
    if result['p'] then
      ...
      zzroom.manager:remove_hand_card(index,0,2)
...
--显示可以操作的界面  --这个界面可以了
function ZZ_Handle:showHandlesView(handle,card)
...
--通知我抓的牌
function ZZ_Handle:SVR_OWN_CATCH_BROADCAST(pack)
  ...
  ZZ_Handle:showTimer(UID,timer_outcard)

  layout_object:drawHandCard(0,true,pack.card)
...
--广播抓牌用户,不知道有没有包括自己
function ZZ_Handle:SVR_PLAYING_UID_BROADCAST(pack)
...
--广播用户出牌
function ZZ_Handle:SVR_SEND_MAJIANG_BROADCAST(pack)
  print("pack-------- 0x4104-------")
  dump(pack)
  if layout == nil then
    return
...
--显示倒计时器
function ZZ_Handle:showTimer(uid,time)
  ...
  layout_object:begin_timer(tip_name,time, index)

end


--用户退出房间
function ZZ_Handle:SVR_QUIT_ROOM(pack)
...
--玩家退出
function ZZ_Handle:SVR_QUICK_SUC(pack)
...
--处理进入房间
function ZZ_Handle:SVR_GET_ROOM_OK(pack_data)
...
--用户重登房间1009
function ZZ_Handle:SVR_REGET_ROOM(pack)
  print("----------------SVR_REGET_ROOM-----------------------------------------")
  dump(pack)
  timer_outcard = pack["nOutCardTimeout"]
  ...
  local gold =  pack["user_gold"] or 0
  local account_object = account.get_player_account()
  ...
  if pack.chupai_seatid == pack.seat_id then --到我出牌
    ...
    if rec_chupai_uid ~= nil then
      ZZ_Handle:showTimer(rec_chupai_uid,timer_outcard)
    end
  end
...
--服务器告知客户端可以进行的操作
function ZZ_Handle:SVR_NORMAL_OPERATE( pack )
  -- body
...
function ZZ_Handle:SVR_GROUP_TIME(pack)
...
--组局排行榜
function ZZ_Handle:SVR_GROUP_BILLBOARD(pack)
  ...
  local action2 = cc.Sequence:create(cc.DelayTime:create(delay_time),cc.Show:create())
  layout_object:runAction(action2)
...
--登录房间成功1007
function ZZ_Handle:SVR_LOGIN_ROOM(pack)
 ...
 -- dump(group_time,"group_time")
  ...
  if user_mount > 0  then
    for i,v in pairs(pack.users_info) do
      self:showPlayer(v)
      --   local other_index = zzroom.manager:get_other_index(v["uid"])
      -- local layout_object = layout.manager:get_layout_object("room")
      -- layout_object:showPlayerWin(other_index, 2001)
    end
  end

  zz_majiangServer:CLI_READY_GAME()
  ...
  timer_outcard = pack["lesstime"]

      -- local layout_object = layout.manager:get_layout_object("room")
      -- layout_object:showPlayerWin(0, 1)
...
--用户进入房间
function ZZ_Handle:SVR_LOGIN_ROOM_BROADCAST(pack)
 ...
 -- dump(pack)
  if pack ~= nil then
    self:showPlayer(pack)
      ...
      -- local layout_object = layout.manager:get_layout_object("room")
      -- layout_object:showPlayerWin(other_index, 1998)
  end
  --layout.hide_layout("roomhandle")
end

--显示其他玩家
function  ZZ_Handle:showPlayer( pack)
  dump(pack,"showPlayer")
  if layout == nil then
    return
  end
...
--广播用户准备
function ZZ_Handle:SVR_USER_READY_BROADCAST(pack)
  if layout == nil then
    return
  end
...
--发牌协议
function ZZ_Handle:SVR_SEND_USER_CARD(pack)
  if layout == nil then
    return
  end
  ...
  local function CallFucnCallback2()
      ...
      layout_object:begin_timer(tip_name,8, other_index)
...
--显示玩家的牌
function ZZ_Handle:showPlayerCards(layout_object, index )
  layout_object:drawHandCard(index)
end

function ZZ_Handle:BROADCAST_USER_IP( pack )
  -- body
    print("------------------0x106A----------------------")
    dump(pack)
...
function ZZ_Handle:G2C_UPDATE_PLAYER_CHIP(pack)
  ...
  for k, v in pairs(pack["playerlist"]) do
    if v["uid"] == uid then
      -- 玩家本人
      account_object:set_account_gold(v["chips"])

      if layout_object then
        layout_object:showPlayerWin(0, v["turn_chips"])
      end
    else
      if layout_object then
        local other_index = zzroom.manager:get_other_index(v["uid"])
        layout_object:updatePlayerGold(other_index, v["chips"])
        layout_object:showPlayerWin(other_index, v["turn_chips"])
      end
    end
  end
end
--出牌错误返回
function ZZ_Handle:SVR_CHUPAI_ERROR(pack)
