...
SERVER[P.SVR_ENDDING_BROADCAST] = {
    ...
    fmt = {
        ...
        {name="count_timeout",type=T.ARRAY,fixedLength = 4,--超时出牌次数
            fmt = {
                {type = T.INT}
            }
        },
...
-- 更新玩家分数
P.G2C_UPDATE_PLAYER_CHIP = 0x8001
SERVER[P.G2C_UPDATE_PLAYER_CHIP] = {
    ver = 1,
    fmt ={
        {name = "playercount",type = T.INT},
        {name = "playerlist", type = T.ARRAY,fixedLengthParser = "playercount",fixedLength = 0,
            fmt = {
                {name = "uid", type = T.INT},
                {name = "chips", type = T.INT},--玩家当前分数
                {name = "turn_chips", type = T.INT},--玩家变动分数
            }
        }      
    }
