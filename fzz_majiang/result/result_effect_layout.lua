...
mrequire("result.result_effect_template")
mrequire("zzroom.deal_card_path")
ResultEffectLayout = class("ResultEffectLayout", result.result_effect_template.result_effect_Template)
function ResultEffectLayout:_do_after_init()
end
...
function ResultEffectLayout:reset_niaocard_data( cards_tbl,zhongcard )
  ...
  local time1= os.time()
    self.result_effect_itembase:removeAllChildren()
	local zhong_tbl = {}
	...
	for _,card_value in pairs(cards_tbl) do
        ...
        if card_value ~= nil and card_value ~= 0 then
            ...
            self.result_effect_itembase:addChild(result_effect_item)
...
function ResultEffectLayout:before_release()
    -- body
     self.result_effect_itembase:removeAllChildren()
     print("ResultEffectLayout:before_release()-----------------")
end