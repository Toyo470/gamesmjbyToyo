...
function ControlPlaneOperator:init(playerType, img, plane, lgPlane, tingHuPlane)
    ...
    light_img:loadTexture("hn_majiang/image/effect/effect_guang.png")
    ...
    effect_3_img:loadTexture("hn_majiang/image/effect/effect_hu03.png")
    ...
    effect_2_img:loadTexture("hn_majiang/image/effect/effect_hu02.png")
    ...
    effect_1_img:loadTexture("hn_majiang/image/effect/effect_hu01.png")
	...
	if plane then
		...
		self.hu_bt:loadTextureNormal("hn_majiang/image/majong_hu_bt_p.png")
		self.hu_bt:setVisible(false)
		plane:addChild(self.hu_bt)
        
        self.huahu_bt = ccui.Button:create()
		self.huahu_bt:loadTextureNormal("hn_majiang/image/majong_huahu_bt_p.png")
		self.huahu_bt:setVisible(false)
		plane:addChild(self.huahu_bt)

		self.gang_bt = ccui.Button:create()
		self.gang_bt:loadTextureNormal("hn_majiang/image/majong_gang_bt_p.png")
		self.gang_bt:setVisible(false)
		plane:addChild(self.gang_bt)

		self.peng_bt = ccui.Button:create()
		self.peng_bt:loadTextureNormal("hn_majiang/image/majong_peng_bt_p.png")
		self.peng_bt:setVisible(false)
		plane:addChild(self.peng_bt)

		self.chi_bt = ccui.Button:create()
		self.chi_bt:loadTextureNormal("hn_majiang/image/majong_chi_bt_p.png")
		self.chi_bt:setVisible(false)
		plane:addChild(self.chi_bt)

		self.guo_bt = ccui.Button:create()
		self.guo_bt:loadTextureNormal("hn_majiang/image/majong_guo_bt_n.png")
		...
		self.liang_bt:loadTextureNormal("hn_majiang/image/majong_liang_bt_p.png")
		...
		local bt_callback = function(sender, event)
			if event == TOUCH_EVENT_ENDED then
				...
				local controlType = HNMJ_CONTROL_TABLE["type"]
				local value = HNMJ_CONTROL_TABLE["value"]
				local gangCards = HNMJ_CONTROL_TABLE["gangCards"]

				if sender == self.hu_bt then
					plane:setVisible(false)
					HNMJ_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_HU), value)
				elseif sender == self.huahu_bt then
				    plane:setVisible(false)
					HNMJ_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_HU), value)
				elseif sender == self.gang_bt then
					if gangCards and table.getn(gangCards) == 1 then
						plane:setVisible(false)
						HNMJ_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_GANG), gangCards[1])
					end
				elseif sender == self.peng_bt then
					plane:setVisible(false)
					HNMJ_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_PENG), value)
				elseif sender == self.left_chi_bt then
					plane:setVisible(false)
					HNMJ_CONTROLLER:control(CHI_TYPE_LEFT, value)
				elseif sender == self.middle_chi_bt then
					plane:setVisible(false)
					HNMJ_CONTROLLER:control(CHI_TYPE_MIDDLE, value)
				elseif sender == self.right_chi_bt then
					plane:setVisible(false)
					HNMJ_CONTROLLER:control(CHI_TYPE_RIGHT, value)
				elseif sender == self.guo_bt then
					plane:setVisible(false)
					HNMJ_CONTROLLER:control(0, value)
				elseif sender == self.chi_bt then
					...
					if chiCount == 1 then
						--todo
						plane:setVisible(false)
						HNMJ_CONTROLLER:control(chiType, value)
					end
				elseif sender == self.liang_bt then
					plane:setVisible(false)
					
					if table.getn(HNMJ_CONTROL_TABLE.gangSeq) > 0 then
						--todo
						HNMJ_CONTROLLER:showLgSelectBox(HNMJ_CONTROL_TABLE.gangSeq)
					else
						-- HNMJ_CONTROLLER:showTingCards(HNMJ_CONTROL_TABLE.tingSeq)
						HNMJ_LG_CARDS = {}
						HNMJ_CONTROLLER:requestLiangGang()
					end
		...
		self.huahu_bt:addTouchEventListener(bt_callback)
	end
		...
		self.commit_bt:addTouchEventListener(function(sender, event)
				if event == TOUCH_EVENT_ENDED then
					...
					for k,v in pairs(self.lg_select_box:getChildren()) do
						if v:getTag() == 0 then
							--todo
							table.insert(HNMJ_LG_CARDS, v.m_value)
						end
					end

					HNMJ_CONTROLLER:requestLiangGang()
					-- for i=table.getn(HNMJ_CONTROL_TABLE.tingSeq),1,-1 do
					-- 	for k,v in pairs(HNMJ_LG_CARDS) do
					-- 		if v == HNMJ_CONTROL_TABLE.tingSeq[i] then
					-- 			--todo
					-- 			table.remove(HNMJ_CONTROL_TABLE.tingSeq, i)
					-- 			break
					-- 		end
					-- 	end
					-- end
...
function ControlPlaneOperator:showImage(img, type)
	...
	if bit.band(type, CONTROL_TYPE_HU) > 0 then
		if  bit.band(type, HU_TYPE_HH)>0 then
		img_type = IMAGE_SHOW_TYPE_HUAHU
		elseif  bit.band(type, HU_TYPE_ZM)>0 then
	...
	effect_1_img:loadTexture("hn_majiang/image/effect/effect_" .. img_type .. "3.png")
	effect_2_img:loadTexture("hn_majiang/image/effect/effect_" .. img_type .. "2.png")
	effect_3_img:loadTexture("hn_majiang/image/effect/effect_" .. img_type .. "1.png")
...
function ControlPlaneOperator:showPlane(plane, controlType)
	...
	if bit.band(controlType, CONTROL_TYPE_HU) > 0 then
		if bit.band(controlType, HU_TYPE_HH) > 0 then
		self.huahu_bt:setVisible(true)
		local size = self.huahu_bt:getSize()
		self.huahu_bt:setPosition(cc.p(oriX + size.width / 2, size.height / 2))
		...
		for i=1,4 do
			...
			card:addTouchEventListener(function(sender, event)
				if event == TOUCH_EVENT_ENDED then
					...
					plane:setVisible(false)
					HNMJ_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_GANG), sender.m_value)
				end
			end)
