...
mrequire("cardhandle.room_card_template")
mrequire("cardhandle.room_out_card_operator")

local Sender = require(GAMEBASENAME..".".."Sender")

RoomCardLayout = class("RoomCardLayout", cardhandle.room_card_template.room_card_Template)
...
function RoomCardLayout:drawHandCard( other_index,listener_flag,zhua_card,need_kongge)
	-- body
	local player_style = zzroom.manager:get_player_style()
	if player_style == 3 and other_index == 2 then
		return
	end
	...
	local fei_card = player_card["fei"] or {}
	...
	if other_index == 0 then
		...
		for card_value,laizi in pairs(fei_card) do
			--normal
			...
			--laizi
			local image =  cardhandle.deal_card_path.get_card_path(laizi)
			local card = self.room_card_peng0:clone()
			card:setPosition(cc.p(x,y))
			...
			ti:loadTexture(GAMEBASENAME.."/res/majiangCard/lai.png")
			card:addChild(ti)
			ti:setPosition(cc.p(34,54))
		...
		local card_height = self.room_card_hand0:getContentSize().height
		...
		local function create_card(_card_value_,_x,_y,_listener_flag )
			-- body
			local image =  cardhandle.deal_card_path.get_card_path(_card_value_)
			...
			local hu_flag = game.hu_dict[other_index]
			if _listener_flag == true and hu_flag ~= 1 then
				self:add_eventlistener(card)
			end
			print("index == 0 ",_card_value_)
			if cardhandle.if_special_card(_card_value_) == true then
				local sp = display.newSprite(GAMEBASENAME.."/res/majiangCard/lai.png")
				card:addChild(sp)
				sp:setPositionX(width-15)
				sp:setPositionY(card_height-20)
			end
		...
		--思路有点乱。我都忘了怎么回事了。这里
		local needge_flag = false
		if table.nums(hand_card) % 3 == 2 and zhua_card == nil then
			needge_flag = true
		end

		if needge_flag == true and cardhandle.last_hand_card == nil then
			needge_flag = false
			need_kongge = true
		end


		local has_deal = false
		for _,card_value in pairs(hand_card) do
			if needge_flag == true and has_deal == false 
			and  card_value == cardhandle.last_hand_card then
				zhua_card = card_value
				has_deal = true
			else 
	...
	elseif other_index == 1 then
		...
		local x = 809.62
		local y = 415
		local height = 22.2--这个高不是控件的高

		zzroom.manager:set_last_pos(1,cc.p(x, y+height))
		table.sort(hand_card,function(v1,v2) return v1 < v2 end)
		...
		if game.hu_card_dict[other_index] ~= nil then
			...
			local image =  cardhandle.deal_card_path.get_card_path(game.hu_card_dict[other_index])
	...
	elseif other_index == 2 then
		...
		for _,card_value in pairs(hand_card) do
			if card_value == 0 then
				...
				x = x - width
			elseif card_value == -1 then
				local back_w = self.room_card_back2:getContentSize().width
				local card = self.room_card_back2:clone()
				card:setPosition(cc.p(x,y))
				self.room_card_image2:addChild(card)
				x = x - back_w+2
	...
	elseif other_index == 3 then
		...
		zzroom.manager:set_last_pos(3,cc.p(x, y))
...
function RoomCardLayout:draw_zhua_card(index,card_value)
  	...
  	if index == 0 and card_value ~= nil then
		...
		card:setTouchEnabled(false)
		local hu_flag = game.hu_dict[index]
		if hu_flag ~= 1 then
			self:add_eventlistener(card)
		end
...
function RoomCardLayout:create_one_out_card( index,card_value,show_dot)
	print(index,card_value,"+++++++++++++++++index,card_value----------------")
		...
		local pos = cardhandle.room_out_card_operator.get_out_card_pos(index,childrenCount + 1)
		card:setAnchorPoint(cc.p(0.5,0.5))
		card:setPosition(pos)
...
function RoomCardLayout:reset_justting_func( pack )
	...
	elseif ting_num_ex > 0 then
		...
		for _ ,card_value_ in pairs(ting_tbl_ex) do
			...
			local image =  cardhandle.deal_card_path.get_card_path(card_value_)
...
function RoomCardLayout:add_eventlistener( card_image )
	...
	local function onTouchBegan(touch, event)
		...
		local room_handle_view_layout_object = layout.manager:get_layout_object("room_handle_view")
		-- if room_handle_view_layout_object ~= nil then
		-- 	return false
		-- end
       ...
       -- dump(rect)
        if cc.rectContainsPoint(rect, locationInNode) and room_handle_view_layout_object == nil then
...
--         return true
    ...
    local function onTouchEnded(touch, event)
        ...
        if math.abs(posY - src_posY ) > 100 and room_handle_view_layout_object == nil  then
			--print("------------出牌-------------")
			local _card_value = target._card_value
			Sender:sendCard(_card_value)
			self:after_send_ting_func(_card_value)
...
function RoomCardLayout:remove_card_event()
	-- body
	print("------------remove_card_event--------------------")
	local eventDispatcher = self:getEventDispatcher()
	local getChildren =  self.room_card_image0:getChildren()
	for _,child in pairs(getChildren) do
    	eventDispatcher:removeEventListenersForTarget(child)
    end
