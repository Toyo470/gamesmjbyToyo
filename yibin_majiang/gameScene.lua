...
GAMEBASENAME = "yibin_majiang"
GAMEBASENAME_RES = GAMEBASENAME.."/res/image/"
...
function ScriptScene:onEnter()
    ...
    mrequire("game.game_operator")
    --game.game_operator.reset_up_jing_list(102,103,4)
    -- game.game_operator.show_jing_result(101,103,"down_jing",{{1,0},{1,2},{0,0},{1,1}})
    -- mrequire("test.test_round_result")
    -- local round_result_dict = test.test_round_result.get_round_result()

    -- zzroom.manager:set_other_seat(10000,0)
    -- zzroom.manager:set_other_seat(10001,1)
    -- zzroom.manager:set_other_seat(10002,2)
    -- zzroom.manager:set_other_seat(10003,3)
    -- game.game_operator.show_round_result(round_result_dict)
    -- local room_card_object = layout.reback_layout_object("room_card")
    -- zzroom.manager:init_card({101,102,103,104,105,106,107,108,109,401,402,403,404,411,412,413})
    ...
    self:init_protocol()
    --cc.Director:getInstance():setDisplayStats(true)--开启调试信息 zsw
  ...
  -- layout.reback_layout_object("seabase")
    ...
    -- local notice_index_list = {}
    -- notice_index_list[131072] = {124,102}
    -- local room_handle_view = layout.reback_layout_object("room_handle_view")
    -- room_handle_view:reset_state(notice_index_list)
    -- local tbl = {
    -- ["conis"] = 24,
    -- ["point"] = 4,
    -- ["target_id_list"] = {},
    -- ["hu_list"] = {"xiaohu","gang","gskh","zimo"}}
    
    -- --local pay_dict = {}

    -- local room_handle_view = layout.reback_layout_object("detail")
    -- room_handle_view:reset_data({["get_list"] = tbl })
end
