 ...
 function ZZ_Handle:G2H_BROADCAST_GAME_RESULT(pack)
   -- body
    ...
    for account_id,content in pairs(account_data) do
      ...
      if zzroom.manager:get_user_data(tonumber(account_hu_conbination_list[1])) ~= nil then
        fangpaoid = tonumber(account_hu_conbination_list[1])
      end

      local hu_com = account_hu_conbination_list[2] or {}
      for _,hu_style in pairs(hu_com) do
        hu_style = result.get_hu_style(hu_style)
      ...
      local other_index = zzroom.manager:get_other_index(tonumber(account_id)) 
      if other_index ~= 0 and room_card_object ~= nil then
          --获取胡的那张牌
          local hucard = account_hu_conbination_list[3] or 0
          hucard = tonumber(hucard)

          local account_handcard = content.account_handcard or {}
          zzroom.manager:set_hand_card_tbl(other_index,account_handcard)
          if account_hu_conbination_list[1] ~= 0 and hucard ~= 0 then
              zzroom.manager:insert_hand_card(other_index,{hucard})
          end
      ...
      if account_hu_conbination_list[1] ~= nil then
        local room_handleresult_object = layout.reback_layout_object("room_handleresult")
        room_handleresult_object:reset_result_hu(other_index)

        music.playEffectHu(account_id)
      end

    end

    dump(hu_contet,"hu_conte")
    local zuan_rec_id =  zzroom.manager:getlast_zuanindex()
    local des_txt = ""
    for account_id,content in pairs(account_data) do
      account_id = tonumber(account_id)
      local other_index = zzroom.manager:get_other_index(tonumber(account_id))
      des_txt = result_object:draw_card(other_index,content)
      des_txt = des_txt .. hu_contet[tonumber(account_id)]
      print("des_txt----------",des_txt)

      if account_id == fangpaoid then
        des_txt = des_txt .. "放炮"
      end

      result_object:reset_result_txt(other_index,des_txt)

      local user_data = zzroom.manager:get_user_data(tonumber(account_id))
      local account_name = user_data["account_name"] or ""
      result_object:reset_name(other_index,account_name)

      if zuan_rec_id == account_id then
         result_object:reset_zuanpos(other_index)
      end
    ...
    local cards_tbl = {}
    local zhongcard = {}
    local game_zhaniao_card =  tbl["game_zhaniao_card"] or {}
    for _card_value,people_list in pairs(game_zhaniao_card) do
      _card_value = tonumber(_card_value)
      table.insert(cards_tbl,_card_value)
      if table.nums(people_list) > 0 then
        table.insert(zhongcard,_card_value)
      end
    end

    result_object:set_niao_card(cards_tbl,zhongcard)

    local use_deley_time = 2  + table.nums(cards_tbl) * 0.5
   ...
   -- result_effect:setVisible(false)
    ...
    -- local function function_name_result_effect()
        layout.hide_layout("seabase")
        
        result_effect:reset_niaocard_data(cards_tbl,zhongcard)
    -- end

    -- local action2 = cc.Sequence:create(cc.DelayTime:create(0),cc.Show:create(),cc.CallFunc:create(function_name_result_effect))
