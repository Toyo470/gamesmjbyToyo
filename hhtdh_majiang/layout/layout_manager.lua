...
function LayoutManager:init_layout_file()
	...
	local FileUtils = require(GAMEBASENAME..".".."fileutils")
	...
	for _,layout_table in pairs(file_table) do
		...
		local LocalZOrder = layout_table["LocalZOrder"] or 0
		if "number" == type(LocalZOrder) then
			self.LocalZOrder_dict[layout_name] = LocalZOrder
		end
	end
...
function LayoutManager:create_layout_object(layout_name, other_index,parent_widget,is_need_game_bg,is_hide_clear,item_layout)
   ...
   -- ui.dump_widget()
	...
	local resourceFilename = GAMEBASENAME.."/res/layout/"..tostring(layout_name) .. ".csb"
	...
	local localz = self.LocalZOrder_dict[layout_name] 
	layout_object:setLocalZOrder(localz)
