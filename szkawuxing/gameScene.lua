require("szkawuxing.globle.SZKWXData")
require("szkawuxing.globle.SZKWXDefine")
...
local receiveHandle = require("szkawuxing.handle.SZKWXReceiveHandle")

local SZKWX_face=require("hall.FaceUI.faceUI")
...
local SZKWXScene = class("SZKWXScene", function()
		return display:newScene()
	end)

local CHILD_NAME_ROOM_LB = "room_lb"

function SZKWXScene:ctor()
	bm.server:setProtocol(PROTOCOL)
    bm.server:setHandle(receiveHandle.new())

	self._scene = cc.CSLoader:createNode("szkawuxing/GameScene.csb"):addTo(self)

    self.reay_my = cc.Sprite:create("szkawuxing/image/mahjong_ready.png")
    self._scene:addChild(self.reay_my)
    self.reay_my:setPosition(cc.p(480, 180))
    self.reay_my:setVisible(false)

    self.reay_right = cc.Sprite:create("szkawuxing/image/mahjong_ready.png")
    self._scene:addChild(self.reay_right)
    self.reay_right:setPosition(cc.p(620, 270))
    self.reay_right:setVisible(false)

    self.reay_left = cc.Sprite:create("szkawuxing/image/mahjong_ready.png")
    ...
    if not bm.Room.SZKWX_uid_seatid then
        bm.Room.SZKWX_uid_seatid = {}
    end



	SZKWX_GAME_PLANE = self._scene:getChildByName("game_plane")

    local SZKWX_face_node = SZKWX_face.new();
    SZKWX_face_node:setHandle(sendHandle)
    self:addChild(SZKWX_face_node, 9999)
    SZKWX_face_node:setName("faceUI")

    dump(SZKWX_GAME_PLANE, "szkawuxing test")

	SZKWX_CONTROLLER = require("szkawuxing.GameController")

	gamePlaneOperator:init()

	SZKWX_MANYOU_PLANE = self._scene:getChildByName(CHILD_NAME_MANYOU_PLANE)
    ...
    share_bt:addTouchEventListener(
        function(sender,event)
            ...
            if event == TOUCH_EVENT_ENDED then
            	...
            	require("hall.common.ShareLayer"):showShareLayer("随州卡五星，房号：" .. USER_INFO["invote_code"], "https://a.mlinks.cc/AK3b", "url", USER_INFO["gameConfig"])
            end
        end
    )
    ...
    local function onface_click(sender)
        ...
        -- else
            SZKWX_face_node:showTxtPanle(pos, 8)
        -- end
...
function SZKWXScene:onEnter()
    ...
    -- require("szkawuxing.RoundEndingLayer"):show(self:testEnding())
    -- require("szkawuxing.operator.GamePlaneOperator"):beginPlayCard(CARD_PLAYERTYPE_MY)
    -- self:showLeaderLayer()
end

function SZKWXScene:get_player_ip( uid )
    -- body
    local ip_ = ""
    ...
    if seatId then
        ...
        if SZKWX_USERINFO_TABLE[seatId .. ""] and SZKWX_USERINFO_TABLE[seatId .. ""].ip then
            --todo
            ip_ = SZKWX_USERINFO_TABLE[seatId .. ""].ip
        end
    end
...
function SZKWXScene:showLeaderLayer()
    local record_bt = self._scene:getChildByName(CHILD_NAME_RECORD_BT)
    require("hall.leader.LeaderLayer"):showLeaderLayer(record_bt:getPosition())
end

function SZKWXScene:ShowChatButton()
    local chat_bt = self._scene:getChildByName(CHILD_NAME_CHAT_BT)
    chat_bt:setVisible(true)
end

function SZKWXScene:ShowRecordButton()
	local record_bt = self._scene:getChildByName(CHILD_NAME_RECORD_BT)
...
--获取用户播放录音位置
function SZKWXScene:getPosforSeat(uid)
    return SZKWX_ROOM.positionTable[uid .. ""]
end

--显示设置按钮
function SZKWXScene:ShowSettingButton()
	local setting_bt = self._scene:getChildByName(CHILD_NAME_SETTING_BT)

	setting_bt:setVisible(true)
end

function SZKWXScene:hideCloseRoomButton()
	local close_room_bt = self._scene:getChildByName(CHILD_NAME_CLOSE_ROOM_BT)

	close_room_bt:setVisible(false)
end

function SZKWXScene:hideShareButton()
	local share_bt = self._scene:getChildByName(CHILD_NAME_SHARE_BT)
	share_bt:setVisible(false)
end

--解散房间
function SZKWXScene:disbandGroup()

	--查询用户当前的游戏状态来判断当前通过怎样的方式来解散组局
    require("hall.gameSettings"):disbandGroup("szkwx")

end

function SZKWXScene:test()
	local cards = {}
	for i=1,13 do
...
function SZKWXScene:testEnding()

	SZKWX_USERINFO_TABLE[0] = {}
	SZKWX_USERINFO_TABLE[0].uid = 723
	SZKWX_USERINFO_TABLE[0].nick = "zh"

	SZKWX_USERINFO_TABLE[1] = SZKWX_USERINFO_TABLE[0]
	SZKWX_USERINFO_TABLE[2] = SZKWX_USERINFO_TABLE[0]
	SZKWX_USERINFO_TABLE[3] = SZKWX_USERINFO_TABLE[0]
...
--聊天
function SZKWXScene:SVR_MSG_FACE(uid, type, sex, node_head, isLeft)
