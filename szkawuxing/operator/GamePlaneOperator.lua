...
local playerPlaneOperator = require("szkawuxing.operator.PlayerPlaneOperator")
local centerPlaneOperator = require("szkawuxing.operator.CenterPlaneOperator")
...
function GamePlaneOperator:init()
	...
	local centerPlane = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_CENTER_PLANE)
	...
	SZKWX_CARD_POINTER = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_CARD_POINTER)

	local remain_card_count_lb = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_REMAIN_CARDS_COUNT_LB)

	remain_card_count_lb:setString("等待玩家进入...")

	local card_bx = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_CARD_BOX)
	card_bx:setVisible(false)

	local remark_lb = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_REMARK_LB)

	if USER_INFO["gameConfig"] then
		--todo
		remark_lb:setString("随州卡五星：" .. USER_INFO["gameConfig"])
	else
...
function GamePlaneOperator:clearGameDatas()
	...
	SZKWX_CARD_POINTER:setPosition(cc.p(-20, -20))
...
function GamePlaneOperator:getPlayerPlane(playerType)
	...
	if playerType == CARD_PLAYERTYPE_MY then
		--todo
		plane = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_MY_PLANE)
	elseif playerType == CARD_PLAYERTYPE_LEFT then
		plane = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_LEFT_PLANE)
	elseif playerType == CARD_PLAYERTYPE_RIGHT then
		plane = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_RIGHT_PLANE)
	elseif playerType == CARD_PLAYERTYPE_TOP then
		plane = SZKWX_GAME_PLANE:getChildByName(CHILD_NAME_TOP_PLANE)
	end
...
function GamePlaneOperator:control(playerType, progCards, controlType)
	dump(SZKWX_CURRENT_CARDNODE, "SZKWX_CURRENT_CARDNODE test")
	if controlType ~= 0x200 and controlType ~= 0x400 then  --除了补杠和暗杠  均不调用
		if not tolua.isnull(SZKWX_CURRENT_CARDNODE) and bit.band(controlType, CONTROL_TYPE_HU) == 0 then
			--todo
			SZKWX_CURRENT_CARDNODE:removeFromParent()

			SZKWX_CARD_POINTER:setVisible(false)
...
function GamePlaneOperator:showControlPlane(controlTable)
	SZKWX_CONTROL_TABLE = controlTable
...
function GamePlaneOperator:showRemainCardsCount()
	...
	if remain_card_count_lb then
		--todo
		local content = "剩余 " .. SZKWX_REMAIN_CARDS_COUNT .. " 张"

		if SZKWX_ROUND then
			--todo
			content = content .. "  第 " .. SZKWX_ROUND .. " / " .. SZKWX_TOTAL_ROUNDS .. " 局"
		end
