...
function ControlPlaneOperator:init(playerType, img, plane, lgPlane, tingHuPlane)
    ...
    light_img:loadTexture("szkawuxing/image/effect/effect_guang.png")
    ...
    effect_3_img:loadTexture("szkawuxing/image/effect/effect_hu03.png")
    ...
    effect_2_img:loadTexture("szkawuxing/image/effect/effect_hu02.png")
    ...
    effect_1_img:loadTexture("szkawuxing/image/effect/effect_hu01.png")
	...
	if plane then
		...
		self.liang_bt:loadTextureNormal("szkawuxing/image/majong_liang_bt_p.png")
		...
		local bt_callback = function(sender, event)
			if event == TOUCH_EVENT_ENDED then
				...
				local controlType = SZKWX_CONTROL_TABLE["type"]
				local value = SZKWX_CONTROL_TABLE["value"]
				local gangCards = SZKWX_CONTROL_TABLE["gangCards"]

				if sender == self.hu_bt then
					--todo
					plane:setVisible(false)
					SZKWX_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_HU), value)
				elseif sender == self.gang_bt then
					if gangCards and table.getn(gangCards) == 1 then
						--todo
						plane:setVisible(false)
						SZKWX_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_GANG), gangCards[1])
					end
				elseif sender == self.peng_bt then
					plane:setVisible(false)
					SZKWX_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_PENG), value)
				elseif sender == self.left_chi_bt then
					plane:setVisible(false)
					SZKWX_CONTROLLER:control(CHI_TYPE_LEFT, value)
				elseif sender == self.middle_chi_bt then
					plane:setVisible(false)
					SZKWX_CONTROLLER:control(CHI_TYPE_MIDDLE, value)
				elseif sender == self.right_chi_bt then
					plane:setVisible(false)
					SZKWX_CONTROLLER:control(CHI_TYPE_RIGHT, value)
				elseif sender == self.guo_bt then
					plane:setVisible(false)
					SZKWX_CONTROLLER:control(0, value)
				elseif sender == self.chi_bt then
					...
					if chiCount == 1 then
						--todo
						plane:setVisible(false)
						SZKWX_CONTROLLER:control(chiType, value)
					end
				elseif sender == self.liang_bt then
					plane:setVisible(false)
					
					if table.getn(SZKWX_CONTROL_TABLE.gangSeq) > 0 then
						--todo
						SZKWX_CONTROLLER:showLgSelectBox(SZKWX_CONTROL_TABLE.gangSeq)
					else
						-- SZKWX_CONTROLLER:showTingCards(SZKWX_CONTROL_TABLE.tingSeq)
						SZKWX_LG_CARDS = {}
						SZKWX_CONTROLLER:requestLiangGang()
					end
	...
	if lgPlane then
		...
		self.commit_bt:addTouchEventListener(function(sender, event)
				if event == TOUCH_EVENT_ENDED then
					...
					for k,v in pairs(self.lg_select_box:getChildren()) do
						if v:getTag() == 0 then
							--todo
							table.insert(SZKWX_LG_CARDS, v.m_value)
						end
					end
		...
		self.lg_liang_bt:addTouchEventListener(function(sender, event)
				if event == TOUCH_EVENT_ENDED and table.getn(componentIndexs) > 0 then
					...
					SZKWX_CONTROLLER:requestLiang(outCard, componentIndexs)
				end
			end)
	end
...
function ControlPlaneOperator:showImage(img, type)
	-- img:setLocalZOrder(9999)		--修改胡效果在听牌下面
	local img_type = ""
	...
	effect_1_img:loadTexture("szkawuxing/image/effect/effect_" .. img_type .. "3.png")
	effect_2_img:loadTexture("szkawuxing/image/effect/effect_" .. img_type .. "2.png")
	effect_3_img:loadTexture("szkawuxing/image/effect/effect_" .. img_type .. "1.png")
		...
		for i=1,4 do
			...
			card:addTouchEventListener(function(sender, event)
				if event == TOUCH_EVENT_ENDED then
					...
					plane:setVisible(false)
					SZKWX_CONTROLLER:control(bit.band(controlType, CONTROL_TYPE_GANG), sender.m_value)
				end
			end)
...
function ControlPlaneOperator:showLgSelectBox(plane, lgCards)  --亮杠操作
		...
		for k1,v1 in pairs(v.component) do
			...
			card:addTouchEventListener(function(sender, event)
				if event == TOUCH_EVENT_ENDED then
					...
					SZKWX_CONTROLLER:showTingHuPlane(CARD_PLAYERTYPE_MY, tingHuCards)
				end
			end)
