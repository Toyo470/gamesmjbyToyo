...
function ManyouPlaneOperator:init()
	SZKWX_MANYOU_PLANE:setVisible(false)

	SZKWX_MANYOU_PLANE:setLocalZOrder(2000)

	local mo_bt = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_MO_BT)
	local bumo_bt = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_BUMO_BT)
	...
	local function bt_callback(sender, event)
		if event == TOUCH_EVENT_ENDED then
			...
			if sender == mo_bt then
				--todo
				SZKWX_CONTROLLER:acceptManyou(true)
				self:hide()
			elseif sender == bumo_bt then
				SZKWX_CONTROLLER:acceptManyou(false)
				self:hide()
			end
		end
	end
...
function ManyouPlaneOperator:show(card, uid)
	SZKWX_MANYOU_PLANE:setVisible(true)
	local light_img = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_LIGHT_IMG)

	local ac = cc.Sequence:create(cc.RotateBy:create(3, 360))
	light_img:runAction(cc.RepeatForever:create(ac))

	local cardNode = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_CARD)
	if cardNode then
		--todo
		cardNode:removeFromParent()
	end
	local bgCard = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_BG_CARD)
	local mo_bt = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_MO_BT)
	local bumo_bt = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_BUMO_BT)
	local desc_lb = SZKWX_MANYOU_PLANE:getChildByName(CHILD_NAME_DESC_LB)
	...
	if card then
		...
		local nick = SZKWX_USERINFO_TABLE[SZKWX_SEAT_TABLE[uid .. ""] .. ""].nick
		...
		SZKWX_MANYOU_PLANE:addChild(newCardNode)

		
		local sequenceAc = cc.Sequence:create(cc.DelayTime:create(2), cc.Hide:create())
		SZKWX_MANYOU_PLANE:runAction(sequenceAc)
	else
