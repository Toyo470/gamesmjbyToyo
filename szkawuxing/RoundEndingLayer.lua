...
function RoundEndingLayer:showZhuaniao(data, isEnd)
	...
	local timeDelay = require("szkawuxing.result_effect_layout"):reset_niaocard_data(cards_tb1, zhongcard)
	...
	if zhuaniaoLayout then
		...
		local callFuncAc = cc.CallFunc:create(function ()
				...
				SZKWX_CONTROLLER:clearGameDatas()
...
function RoundEndingLayer:show(data, isEnd)
	...
	self._scene = cc.CSLoader:createNode("szkawuxing/roundEndingLayer.csb")
	...
	for i=1,table.getn(players) do
		...
		if SZKWX_ZHUANG_UID == players[i].uid then
			--todo
			isZhuang = true
		end
	...
	if close_bt then
		...
		local function bt_callback(sender, event)
			if event == TOUCH_EVENT_ENDED then
				...
				if SZKWX_GROUP_ENDING_DATA then
					--todo
...
function RoundEndingLayer:showPlayerPlane(player, index, isZhuang, huCard, endType)
	...
	local nickname = SZKWX_USERINFO_TABLE[seatId .. ""].nick
...
function RoundEndingLayer:showEndingInfo(player, index)
	...
	for i=1,player.huTypeCount do
		endingInfo = endingInfo .. SZKWX_HU_TYPE_TABLE[player.huTypes[i] + 1] .. " "
	end
...
function RoundEndingLayer:showTitle(titleType)
	...
	if titleType == TITLE_TYPE_WIN then
		--todo
		title_bg_img:loadTexture("szkawuxing/image/red_button_p.png")
		title_img:loadTexture("szkawuxing/image/mahjong_win.png")
	elseif titleType == TITLE_TYPE_LOSE then
		title_bg_img:loadTexture("szkawuxing/image/mahjong_lose_bt.png")
		title_img:loadTexture("szkawuxing/image/mahjong_lose.png")
	elseif titleType == TITLE_TYPE_LIUJU then
		title_bg_img:loadTexture("szkawuxing/image/grey_button.png")
		title_img:loadTexture("szkawuxing/image/text_liuju.png")
	else
		title_bg_img:loadTexture("szkawuxing/image/mahjong_lose_bt.png")
		title_img:loadTexture("szkawuxing/image/pingju.png")
	end
end
...
function RoundEndingLayer:showGameRemark(timeStr)
	...
	if USER_INFO["gameConfig"] then
		--todo
		bird_lb:setString("随州卡五星：" .. USER_INFO["gameConfig"])
	else
