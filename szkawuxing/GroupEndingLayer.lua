...
function GroupEndingLayer:showGroupResult(pack)
	...
	--获取总结算界面
    local s = cc.CSLoader:createNode("szkawuxing/GameResult.csb")
    ...
    if #playerlist > 0 then
    	...
    	for k,v in pairs(playerlist) do
	    	...
	    	--定义结果分项
	    	local result_Item = cc.CSLoader:createNode("szkawuxing/GameResult_Item.csb")
	    	...
	    	if user_info.zimo_dahu ~= nil then
	    		dhzm_itemNum_tt:setString(tostring(user_info.zimo_dahu))
	    	end

	    	--点炮
	    	local xhzm_ly = content_sv:getChildByName("xhzm_ly")
	    	local xhzm_itemNum_tt = xhzm_ly:getChildByName("itemNum_tt")
	    	if user_info.zimo_xiaohu ~= nil then
	    		xhzm_itemNum_tt:setString(tostring(user_info.zimo_xiaohu))
	    	end

	    	--接炮
	    	local dhdp_ly = content_sv:getChildByName("dhdp_ly")
	    	local dhdp_itemNum_tt = dhdp_ly:getChildByName("itemNum_tt")
	    	if user_info.dianpao_dahu ~= nil then
	    		dhdp_itemNum_tt:setString(tostring(user_info.dianpao_dahu))
	    	end

	    	--暗杠
	    	local xhdp_ly = content_sv:getChildByName("xhdp_ly")
	    	local xhdp_itemNum_tt = xhdp_ly:getChildByName("itemNum_tt")
	    	if user_info.dianpao_xiaohu ~= nil then
	    		xhdp_itemNum_tt:setString(tostring(user_info.dianpao_xiaohu))
	    	end

	    	--明杠
	    	local dhjp_ly = content_sv:getChildByName("dhjp_ly")
	    	local dhjp_itemNum_tt = dhjp_ly:getChildByName("itemNum_tt")
	    	if user_info.jiepao_dahu ~= nil then
	    		dhjp_itemNum_tt:setString(tostring(user_info.jiepao_dahu))
	    	end
