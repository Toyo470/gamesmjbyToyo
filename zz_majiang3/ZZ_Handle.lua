...
local zz_majiang3Server = require("zz_majiang3.zz_majiangServer")

local hallPa = require("hall.HALL_PROTOCOL")
local PROTOCOL = import("zz_majiang3.ZZ_Protocol")
  ...
  if applyId == uid then
    ...
    if isMyAgree == 1 then
      ...
      tips.show_tips_dissove("提示", showMsg,function()
        -- body
        zz_majiang3Server:c2sdissove_room(1)
      end,
      function()
        -- body
        zz_majiang3Server:c2sdissove_room(0)
      end,true)
    end
...
function ZZ_Handle:SVR_HUPAI_BROADCAST(pack)
 ...
 -- local layout_object = nil
  ...
  for i ,content in pairs(pack.content) do
    ...
    -- end
    --if handpeng then
      require("zz_majiang3.pengresult"):show_hu_result(index)
    --end
...
function ZZ_Handle:SVR_PLAYER_USER_BROADCAST(pack)
  ...
  if pack.uid == tonumber(UID) then
    ...
    if result['g'] or result['pg'] then
      ...
      require("zz_majiang3.pengresult"):show_handle_result(index,result)
    end 
...
function ZZ_Handle:SVR_REGET_ROOM(pack)
  ...
  if SCENENOW["name"] == "zz_majiang3.gameScene" then
...
function ZZ_Handle:SVR_LOGIN_ROOM(pack)
 ...
 -- dump(group_time,"group_time")
  ...
  zz_majiang3Server:CLI_READY_GAME()
