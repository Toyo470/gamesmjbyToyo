local XYKWXData = require("xykawuxing.globle.XYKWXData")

local CardUtils = class("CardUtils")

function CardUtils:insertCardsByUid(uid, newCard)
	--todo
	local cards = XYKWXData:getCards(uid)
...
function CardUtils:getPlayerType(seatId)
    local other_index = seatId - XYKWX_MY_USERINFO.seat_id
...
function CardUtils:removeCards(seatId, removeCards)
	local oriCards = XYKWX_GAMEINFO_TABLE[seatId .. ""].hand
...
function CardUtils:getNewCard(seatId, value)
	...
	dump(XYKWX_GAMEINFO_TABLE, "getNewCard XYKWX_GAMEINFO_TABLE test")
