require("xykawuxing.globle.XYKWXData")
require("xykawuxing.globle.XYKWXDefine")
...
local receiveHandle = require("xykawuxing.handle.XYKWXReceiveHandle")

local XYKWX_face=require("hall.FaceUI.faceUI")
...
local XYKWXScene = class("XYKWXScene", function()
		return display:newScene()
	end)

local CHILD_NAME_ROOM_LB = "room_lb"

function XYKWXScene:ctor()
	bm.server:setProtocol(PROTOCOL)
    bm.server:setHandle(receiveHandle.new())

	self._scene = cc.CSLoader:createNode("xykawuxing/GameScene.csb"):addTo(self)

    self.reay_my = cc.Sprite:create("xykawuxing/image/mahjong_ready.png")
    self._scene:addChild(self.reay_my)
    self.reay_my:setPosition(cc.p(480, 180))
    self.reay_my:setVisible(false)

    self.reay_right = cc.Sprite:create("xykawuxing/image/mahjong_ready.png")
    self._scene:addChild(self.reay_right)
    self.reay_right:setPosition(cc.p(620, 270))
    self.reay_right:setVisible(false)

    self.reay_left = cc.Sprite:create("xykawuxing/image/mahjong_ready.png")
    ...
    if not bm.Room.XYKWX_uid_seatid then
        bm.Room.XYKWX_uid_seatid = {}
    end


	XYKWX_GAME_PLANE = self._scene:getChildByName("game_plane")

    local XYKWX_face_node = XYKWX_face.new();
    XYKWX_face_node:setHandle(sendHandle)
    self:addChild(XYKWX_face_node, 9999)
    XYKWX_face_node:setName("faceUI")

    dump(XYKWX_GAME_PLANE, "xykawuxing test")

	XYKWX_CONTROLLER = require("xykawuxing.GameController")

	gamePlaneOperator:init()

	XYKWX_MANYOU_PLANE = self._scene:getChildByName(CHILD_NAME_MANYOU_PLANE)
    ...
    share_bt:addTouchEventListener(
        function(sender,event)
            ...
            if event == TOUCH_EVENT_ENDED then
            	...
            	require("hall.common.ShareLayer"):showShareLayer("襄阳卡五星，房号：" .. USER_INFO["invote_code"], "https://a.mlinks.cc/AK3b", "url", USER_INFO["gameConfig"])
            end
        end
    )
    ...
    local function onface_click(sender)
        ...
        -- else
            XYKWX_face_node:showTxtPanle(pos, 8)
        -- end
...
function XYKWXScene:onEnter()
    ...
    -- require("xykawuxing.RoundEndingLayer"):show(self:testEnding())
    -- require("xykawuxing.operator.GamePlaneOperator"):beginPlayCard(CARD_PLAYERTYPE_MY)
    -- self:showLeaderLayer()
end

function XYKWXScene:get_player_ip( uid )
    -- body
    local ip_ = ""
    ...
    if seatId then
        ...
        if XYKWX_USERINFO_TABLE[seatId .. ""] and XYKWX_USERINFO_TABLE[seatId .. ""].ip then
            --todo
            ip_ = XYKWX_USERINFO_TABLE[seatId .. ""].ip
        end
    end
...
function XYKWXScene:showLeaderLayer()
    local record_bt = self._scene:getChildByName(CHILD_NAME_RECORD_BT)
    require("hall.leader.LeaderLayer"):showLeaderLayer(record_bt:getPosition())
end

function XYKWXScene:ShowChatButton()
    local chat_bt = self._scene:getChildByName(CHILD_NAME_CHAT_BT)
    chat_bt:setVisible(true)
end

function XYKWXScene:ShowRecordButton()
	local record_bt = self._scene:getChildByName(CHILD_NAME_RECORD_BT)
...
--获取用户播放录音位置
function XYKWXScene:getPosforSeat(uid)
    return XYKWX_ROOM.positionTable[uid .. ""]
end

--显示设置按钮
function XYKWXScene:ShowSettingButton()
	local setting_bt = self._scene:getChildByName(CHILD_NAME_SETTING_BT)

	setting_bt:setVisible(true)
end

function XYKWXScene:hideCloseRoomButton()
	local close_room_bt = self._scene:getChildByName(CHILD_NAME_CLOSE_ROOM_BT)

	close_room_bt:setVisible(false)
end

function XYKWXScene:hideShareButton()
	local share_bt = self._scene:getChildByName(CHILD_NAME_SHARE_BT)
	share_bt:setVisible(false)
end

--解散房间
function XYKWXScene:disbandGroup()

	--查询用户当前的游戏状态来判断当前通过怎样的方式来解散组局
    require("hall.gameSettings"):disbandGroup("xykwx")

end

function XYKWXScene:test()
	local cards = {}
	for i=1,13 do
...
function XYKWXScene:testEnding()

	XYKWX_USERINFO_TABLE[0] = {}
	XYKWX_USERINFO_TABLE[0].uid = 723
	XYKWX_USERINFO_TABLE[0].nick = "zh"

	XYKWX_USERINFO_TABLE[1] = XYKWX_USERINFO_TABLE[0]
	XYKWX_USERINFO_TABLE[2] = XYKWX_USERINFO_TABLE[0]
	XYKWX_USERINFO_TABLE[3] = XYKWX_USERINFO_TABLE[0]
...
--聊天
function XYKWXScene:SVR_MSG_FACE(uid, type, sex, node_head, isLeft)
