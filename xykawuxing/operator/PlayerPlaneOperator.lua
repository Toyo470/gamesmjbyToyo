local inhandPlaneOperator = require("xykawuxing.operator.InhandPlaneOperator")
local lefthandPlaneOperator = require("xykawuxing.operator.LefthandPlaneOperator")
local controlPlaneOperator = require("xykawuxing.operator.ControlPlaneOperator")
local outhandPlaneOperator = require("xykawuxing.operator.OuthandPlaneOperator")
local userInfoPlaneOperator = require("xykawuxing.operator.UserInfoPlaneOperator")
local jiapiaoPlaneOperator = require("xykawuxing.operator.JiapiaoPlaneOperator")
...
function PlayerPlaneOperator:showCards(playerType, plane)
	...
	local seatId = XYKWX_SEAT_TABLE_BY_TYPE[playerType .. ""]

	local cards = XYKWX_GAMEINFO_TABLE[seatId .. ""].hand
	local tingFlag = XYKWX_GAMEINFO_TABLE[seatId .. ""].ting
	local anke = XYKWX_GAMEINFO_TABLE[seatId .. ""].anke
	local liangCards = XYKWX_GAMEINFO_TABLE[seatId .. ""].liangCards

	-- if tingFlag ~= 1 or XYKWX_ROOM.isBufenLiang == 1 then
	if tingFlag ~= 1 then
		--todo
...
function PlayerPlaneOperator:redrawGameInfo(playerType, plane, data)
	...
	-- if playerType == CARD_PLAYERTYPE_MY then
		--todo
		local tingFlag = XYKWX_GAMEINFO_TABLE[data.seat_id .. ""].ting
...
function PlayerPlaneOperator:showTingCards(plane, tingSeq)
	...
	local seatId = XYKWX_MY_USERINFO.seat_id
...
function PlayerPlaneOperator:hideTingHuPlane(plane)
	local tingHuPlane = plane:getChildByName(CHILD_NAME_TING_HU_PLANE)
	local tingFlag = XYKWX_GAMEINFO_TABLE[XYKWX_MY_USERINFO.seat_id .. ""].ting
...
function PlayerPlaneOperator:showPiaoImg(plane, piao, flag)
	...
	if flag then
		...
		piaoImg:loadTexture("xykawuxing/image/" .. imgPath)
	end
end
