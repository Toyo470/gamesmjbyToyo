		    	...
		    	for k,v in pairs(children) do
					...
					if cc.rectContainsPoint(rect, locationInNode) then
			            ...
			            if card_click_flag == true then
			            	XYKWX_CONTROLLER:playCard(cardTemp.m_value)
			            	return false
			            else
...
function InhandPlaneOperator:cancelSelectingCard(plane)
	...
	XYKWX_CONTROLLER:hideTingHuPlane()
			...
			if isTing then
				...
				if event == TOUCH_EVENT_ENDED then
					...
					if selectedTag == sender:getTag() then
							...
							-- XYKWX_CONTROLLER:requestLiang(sender.m_value)
							...
							for k,v in pairs(tingSeq) do
								if v.card == sender.m_value then
									--todo
									XYKWX_CONTROLLER:showComponentSelectBox(v)
