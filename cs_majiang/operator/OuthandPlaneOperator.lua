...
function OuthandPlaneOperator:playCard(playerType, cardValue, plane)
	...
	if playerType == CARD_PLAYERTYPE_MY then
		...
		local function card_callback()  
		   ...
		   CSMJ_CARD_POINTER:setPosition(cc.p(worldPoint.x + cardNode:getSize().width / 2, worldPoint.y + cardNode:getSize().height + CSMJ_CARD_POINTER:getSize().height / 4))
		   CSMJ_CARD_POINTER:setVisible(true)

		   local pointPosition = CSMJ_CARD_POINTER:getPosition()
		   ...
		   CSMJ_CARD_POINTER:runAction(cc.RepeatForever:create(seqAcT))
	...
	CSMJ_CURRENT_CARDNODE = cardNode
end
