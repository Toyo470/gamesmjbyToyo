...
-- local pdkSettings=import(".pdkSettings")

--动画数据
faceUI.animatePlist={
    plist="hall/FaceUI/faceAnimate.plist",
    faceSpeed=8/32,
    ...
    imageUse="hall/FaceUI/expression_box.png"
}
...
function faceUI:ctor()
    ...
    --父容器
    self.rootNode=cc.CSLoader:createNode("hall/FaceUI/faceUI.csb")
...
--------------------------------------------------适配卡五星框架点击空白处隐藏表情框体------------------------------------
    ...
    cc.Director:getInstance():getEventDispatcher():addEventListenerWithSceneGraphPriority(listener2:clone(),touchlayer)
 ---------------------------------------------------------------------------------------------------------------------------

    self:hidePanle()
end

function faceUI:setHandle(handle)
    sendHandle = handle
end
...
--广播显示动画
function faceUI:showGetFace(uid,faceid,sex,node_head,isLeft)
    ...
    -- local name,index=pdkSettings:getDOS(seat)

    -- local node_head=SCENENOW["scene"]._scene:getChildByName(name)
    -- local pos=node_head:getPosition()
    local pos = node_head:convertToWorldSpace(cc.p(0, 0))
    -- local pos = position
   ...
   if faceid>=50 then --玩家发过来的表情
        ...
        if sex == 0 or sex == 1 then
            strSound = "hall/FaceUI/audio/Man_Chat_"..tostring(faceid-1)..".mp3"
        elseif sex == 2 then
            strSound = "hall/FaceUI/audio/Woman_Chat_"..tostring(faceid-1)..".mp3"
        end
        if strSound ~= "" then
            print("showGetFace",strSound)
            -- audio.pauseMusic()
            require("hall.GameCommon"):playEffectSound(strSound)
        end
	end
    ...
    print("showGetFace end")
end

-- posType 1：左上 2：右上 3：左下 4：右下
function faceUI:showTxtPanle(pos, posType)
    pos.x=pos.x
    if posType then
        --todo
        if posType == 1 then
        ...
        elseif posType == 5 then
            pos.x=pos.x + 20
            pos.y=pos.y - 310
        elseif posType == 6 then
            pos.x=pos.x - 235
            pos.y=pos.y - 310
        elseif posType == 7 then
            pos.x=pos.x - 470
            pos.y=pos.y - 310
        elseif posType == 8 then
            pos.x=pos.x - 470
            pos.y=pos.y - 155
        end
    end
