...
function RoundResultLayout:refresh_change_chips(game_account_data_dict)
    for account_id,account_data_dict in pairs(game_account_data_dict) do
        ...
        print("account_id ",account_id,"----seat_index-",seat_index,"----account_change_chip-",account_change_chip)
...
function RoundResultLayout:ini_my_card( game_account_data_dict )
    ...
    for account_id,_ in pairs(game_account_data_dict) do
      ...
      local id_name = ui.manager:get_widget_object("round_result_account_id"..tostring(seat_index))
      if id_name ~= nil then
         id_name:setString(tostring(account_id))
      end

      local nick_name = player_data["account_name"] or ""
      local name_icon = ui.manager:get_widget_object("round_result_account_name"..tostring(seat_index))
      if name_icon ~= nil then
          name_icon:setString(tostring(nick_name))
      end

      local head_photo=  ui.manager:get_widget_object("round_result_head_photo"..tostring(seat_index))

      local w = head_photo:getContentSize().width
      local h = head_photo:getContentSize().height
      local sp = display.newSprite(GAMEBASENAME .. "/res/image/head_box2.png")
      sp:setAnchorPoint(cc.p(0.5,0.5))
      head_photo:addChild(sp)
