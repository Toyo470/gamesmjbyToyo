...
function RoomBaseLayout:_do_after_init()
	...
	self.room_base_gamemsg:setString("赣州通关：".. tostring(gameConfig))
	...
	self.up_jing_bg_widget_list = {self.room_base_up_zj_bg,self.room_base_up_fj_bg}--上精背景列表
	self.up_jing_img_widget_list = {self.room_base_up_zj_img,self.room_base_up_fj_img}--上精圖片列表

	self.down_jing_bg_s_widget_list = {self.room_base_down_s_zj_bg,self.room_base_down_s_fj_bg}--单个下精背景列表

	self.down_jing_img_s_widget_list = {self.room_base_down_s_zj_img,self.room_base_down_s_fj_img}--单个下精图片列表

	self.down_jing_flag_s_widget_list = {self.room_base_down_s_zj_flag,self.room_base_down_s_fj_flag}--单个下精标示列表

	self.left_down_right_jing_bg_widget_list = {self.room_base_left_jing_bg,self.room_base_down_jing_bg,
													self.room_base_right_jing_bg}--下左右背景列表

	self.left_down_right_jing_img_widget_list = {self.room_base_left_jing_img,self.room_base_down_jing_img,
												self.room_base_right_jing_img}--左下右精图片列表

	self.left_down_right_jing_flag_widget_list = {self.room_base_left_jing_flag,self.room_base_down_jing_flag,
												self.room_base_right_jing_flag}--左下右标示列表
	...
	self.room_base_left_top:setVisible(false)
	self.room_base_right_top:setVisible(false)
...
function RoomBaseLayout:init_jing_card(jing_count)
	self.room_base_left_top:setVisible(true)
	self.room_base_right_top:setVisible(true)
	self:show_jing_widget_visible(self.up_jing_bg_widget_list,false)
	self:show_jing_widget_visible(self.down_jing_bg_s_widget_list,false)
	self:show_jing_widget_visible(self.left_down_right_jing_bg_widget_list,false)

	self:show_jing_widget_visible(self.left_down_right_jing_img_widget_list,false)
	self:show_jing_widget_visible(self.left_down_right_jing_flag_widget_list,false)
	self:show_jing_widget_visible(self.down_jing_img_s_widget_list,false)
	self:show_jing_widget_visible(self.down_jing_flag_s_widget_list,false)
	
	self.jing_count = jing_count

	if jing_count == 2 then
		self.room_base_left_jing_text:setVisible(false)--左精字体
		self.room_base_right_jing_text:setVisible(false)--右精字体
	else
		self.room_base_left_jing_text:setVisible(true)
		self.room_base_right_jing_text:setVisible(true)
	end
end
...
function RoomBaseLayout:show_up_jing()
	...
	self:show_jing_widget_visible(self.up_jing_bg_widget_list,true)
	self:show_jing_card_img_list(self.up_jing_img_widget_list,up_jing_card_list)

	if self.jing_count == 2 then
		self:show_jing_widget_visible(self.down_jing_bg_s_widget_list,true)
		self:show_jing_widget_img(self.down_jing_bg_s_widget_list,self.bg_card_image_path)
	else
		self:show_jing_widget_visible(self.left_down_right_jing_bg_widget_list,true)
		self:show_jing_widget_img(self.left_down_right_jing_bg_widget_list,self.bg_card_image_path)
	end
