...
function RoomHandleResultLayout:_do_after_init()
	self.pos = {cc.p(656.02,307.74),cc.p(464.86,406.29),cc.p(273.23,307.74)}
...
function RoomHandleResultLayout:reset_result_item( index,result_tbl,show_time )
	...
	if result_tbl == handleview.FANG_GANG_OPCODE
	or result_tbl == handleview.AN_GANG_OPCODE
	or result_tbl == handleview.MING_GANG_OPCODE
	or result_tbl == handleview.GANG_OPCODE then
	...
	elseif result_tbl == handleview.PENG_OPCODE then
	...
	elseif result_tbl == handleview.CHI_OPCODE then

		self.room_handleresult_c:setPosition(pos)
		self.room_handleresult_c:setScale(0.8)
		local action1= cc.ScaleTo:create(0.5,1.2)

 		local showTurnAc = cc.CallFunc:create(function()
				local fo = cc.FadeOut:create(0.5)
				self.room_handleresult_chi_base:runAction(fo)
 			end)
 		local action2 = cc.ScaleTo:create(0.5,1.0)
 		local sq = cc.Sequence:create(action1,action2,showTurnAc)
 		self.room_handleresult_c:runAction(sq)
	end
