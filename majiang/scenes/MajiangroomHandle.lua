...
--定义麻将游戏界面名称
local run_scene_name = "majiang.scenes.MajiangroomScenes"
...
function MajiangroomHandle:ctor()
	...
	local  func_ = {
		...
		--当前排名--游戏开始
		[PROTOCOL.SVR_GAME_START] = {handler(self, MajiangroomHandle.SVR_GAME_START)},
		--房间不存在
        [PROTOCOL.SVR_ENTER_PRIVATE_ROOM] = {handler(self, MajiangroomHandle.SVR_ENTER_PRIVATE_ROOM)},
		--用户自己退出成功
        ...
        --出牌错误返回
        [PROTOCOL.SVR_CHUPAI_ERROR] = {handler(self, MajiangroomHandle.SVR_CHUPAI_ERROR)},
        --svr通知我抓的牌
		...
		--同时，已经报名的玩家会收到其他玩家进入的消息
     	...
     	--广播玩家IP
     	[PROTOCOL.BROADCAST_USER_IP] = {handler(self, MajiangroomHandle.BROADCAST_USER_IP)},
     	--服务器返回组局收到的信息
     	[PROTOCOL.SERVER_CMD_FORWARD_MESSAGE] = {handler(self, MajiangroomHandle.SERVER_CMD_FORWARD_MESSAGE)},
     	--语音和视屏
     	[PROTOCOL.SERVER_CMD_MESSAGE] = {handler(self, MajiangroomHandle.SERVER_CMD_MESSAGE)},
...
--出牌错误返回
function MajiangroomHandle:SVR_CHUPAI_ERROR(pack)
	...
	elseif errorCode == 3 then
		require("hall.GameCommon"):showAlert(true, "还没到你出牌哦", 200)
	elseif errorCode == 4 then
		require("hall.GameCommon"):showAlert(true, "你手上无此牌", 200)
	elseif errorCode == 5 then
		require("hall.GameCommon"):showAlert(true, "请打缺门牌", 200)
		bm.Room.my_out_card_time = 1 -- 重新出牌
	end
end
...
function MajiangroomHandle:showTimer(uid,time)
	...
	--庄家指示器指向调整
	scenes:show_timer_index(index)--获取玩家显示的位置号 0自己，1左边玩家，2对家，3右边玩家
...
-- 游戏开始
function MajiangroomHandle:SVR_GAME_START( pack )
	-- body
...
function MajiangroomHandle:SVR_LOGIN_ROOM(pack)
	...
	--绘制自己的信息
	scenes:set_ower_info(USER_INFO["icon_url"],USER_INFO["nick"],pack["gold"] - USER_INFO["group_chip"],USER_INFO["sex"])
	...
	--再来一局的情况下隐藏邀请、解散按钮
	if bm.playAgainFlag then
		scenes:setBtns(false)
		bm.playAgainFlag = false
	else
		scenes:setBtns(true)
	end
...
function MajiangroomHandle:SVR_REGET_ROOM(pack)
	...
	if pack.pHuSeatId == 1 then
		...
		--显示自己胡后整理好的牌
		self:drawPlayerHupaiCard(tonumber(UID), pack.pHudate_c, pack.pHudate_p)
		...
		dump(_card_num,"88888888888888888888888888888888888888888888888888888888888888888888")
		--判断是否自己出牌
		if  _card_num == 2 or  
			_card_num == 5 or 
			_card_num == 8 or 
	...
	--显示当前出牌位置
	scenes:show_timer_index(pack.chupai_seatid)

	--轮到本人出牌
	if pack.chupai_seatid == pack.seat_id then
 ...
 --    if require("hall.gameSettings"):getGameMode() == "group" then
	...
	if pack.myNowHandle ~= nil then
		self:showHandlesView(pack.myNowHandle,pack.lastCard,pack.canGangArr)
	end
	...
	SCENENOW["scene"]:set_left_card_num(pack.card_less)

	if pack.isChangeCard and pack.isChangeCard==1 then
		self:SERVER_COMMAND_NEED_CHANGE_CARD()
	else
		bm.Room.change_card_over = true
	end
...
function MajiangroomHandle:showPlayer(pack)
    ...
    --显示其他用户到界面
   	...
   	self:setNetworkOffline(other_index, false)
    ...
    if bm.Room.isStart == 1 then
    	scenes:show_hasselect(other_index, false)
    end
...
function MajiangroomHandle:setNetworkOffline( index, flag )
	local scenes = SCENENOW['scene'] 
	if SCENENOW['name'] ~= run_scene_name then
		return
	end
	...
	local p
	if index == 0 then
		panel = scenes._scene:getChildByName("Panel_me")
		p = cc.p(50,100)
	elseif index == 1 then
		panel = scenes._scene:getChildByName("Panel_left") 
		p = cc.p(50,160)
	elseif index == 2 then
		panel = scenes._scene:getChildByName("Panel_top") 
		p = cc.p(50,100)
	elseif index == 3 then
		panel = scenes._scene:getChildByName("Panel_right") 
		p = cc.p(50,160)
	end

	local network = panel:getChildByName("network")
	if not tolua.isnull(network) then
		network:removeSelf()
	end
	if flag then
		network = cc.Sprite:create("majiang/image/duanxianchonglian_bt.png")
		panel:addChild(network)
		network:setName("network")
		network:setPosition(p)
...
function MajiangroomHandle:SVR_QUIT_ROOM(pack)
	...
	self:setNetworkOffline(index, true)
...
--显示可以操作的界面
function MajiangroomHandle:showHandlesView(handle,card,cards)
	...
	if result['g'] and ifpg == 0 then
		...
		if cards ~= nil then
			...
			--计算有几个杠
			local gang_count = 0
			local selectCard_arr = {}
			for i,v in ipairs(cards) do
				if v > 0 then
					gang_count = gang_count + 1
					table.insert(selectCard_arr, v)
				end
			end

			if gang_count > 1 then
				--多个杠，则出现选牌杠区域
				require("majiang.view.SelectCardView"):showView(420, 155, result['g'], selectCard_arr)

			else
				--只有一个杠，则杠标签可点
				bm.nodeClickHandler(p, MajiangroomServer.requestHandle, result['g'], selectCard_arr[1])
	...
	if has == 0 then
        ...
        guo:addNodeEventListener(cc.NODE_TOUCH_EVENT,function ( event )
                if event.name == "began" then
                	...
                	require("majiang.view.SelectCardView"):removeView()
...
function MajiangroomHandle:MypengHuGangAnim(result)
	...
	if result == "p" then
		...
		--添加底图
		local path_other_peng = "majiang/image/effect_peng03.png"
		...
		--添加顶层
		local path_other_peng_t = "majiang/image/effect_peng01.png"
...
function MajiangroomHandle:pengHuGangAnim(uid,result)
	...
	local scenes = SCENENOW['scene']
	local index  = self:getIndex(uid)
	print("pengHuGangAnim", tostring(index), tostring(uid))
	...
	if result['g'] or result['pg'] then
		...
		local path_other_gang = "majiang/image/effect_gang03.png"
		local object = display.newSprite(path_other_gang)
		object:pos(config[index]['x'],config[index]['y'])
		object:addTo(node)

		local path_other_gang_t = "majiang/image/effect_gang01.png"
	...
	if result['h']then
		...
		local path_other_hu = "majiang/image/effect_hu03.png"
		local object = display.newSprite(path_other_hu)
		object:pos(config[index]['x'],config[index]['y'])
		object:addTo(node)

		local path_other_hu_t = "majiang/image/effect_hu01.png"
...
function MajiangroomHandle:SVR_SEND_USER_CARD(pack)
    ...
    if require("hall.gameSettings"):getGameMode() == "group" then
        --todo
        require("majiang.gameScene"):setGameState(2)
    end
...
function MajiangroomHandle:drawHandCard(index)
		...
		if bm.User.Que ~= nil then
			if bm.Room.Card[index]['hand'] ~= nil then
				...
				for k,v in pairs(bm.Room.Card[index]['hand']) do
					...
					local nowCardVariety = require("majiang.card.Majiangcard"):getVariety(v)
					--判断当前牌的花色是否为缺色
	...
	if index == 2 then
		...
		-- local y_ponit = zhua.00
		local y_ponit = 470.00
...
function MajiangroomHandle:drawHandCardAfterBack(index)
	...
	for i,v in pairs(bm.Room.Card[index]['hand']) do 
		...
		local tmp = Majiangcard.new()
		tmp:setCard(v)
		dump(bm.Room.change_card_over,"重连设置自己的手牌")
		bm.Room.change_card_over = true
		tmp:setMyHandCard()
		tmp:setScale(1.1)
		-- tem:setScaleY()
...
function MajiangroomHandle:drawOut(index)
	...
	local path_nowCardSign = "majiang/image/nowCardSign.png"
	...
	if index == 0 then
		...
		local y_start1 = 215.00

		local x_start2 = 391.00
		local y_start2 = 183.00

		local x_start3 = 364.00
		local y_start3 = 152.00

		local x_start4 = 337.00
		local y_start4 = 119.00
	...
	if index == 1 then
		...
		local x_start1 = 317.00
		local y_start1 = 339.00

		local x_start2 = 281.00
		local y_start2 = 362.00

		local x_start3 = 245.00
		local y_start3 = 385.00

		local x_start4 = 209.00
	...
	if index == 3 then
		...
		local x_start1 = 615.00
		local y_start1 = 251.00

		local x_start2 = 651.00
		local y_start2 = 228.00

		local x_start3 = 687.00
		local y_start3 = 207.00

		local x_start4 = 723.00
...
function MajiangroomHandle:SVR_OWN_CATCH_BROADCAST(pack)
	...
	--检查和显示我可以做的操作
	self:showHandlesView(pack.handle, pack.card, pack.cards)

	--显示操作倒计时
	MajiangroomHandle:showTimer(tonumber(UID),8)
...
function MajiangroomHandle:showRoundResult(pack)
	...
	if bm.round == bm.round_total then
		again_ly:getChildByName("Image_5"):loadTexture("majiang/common/text_youxijieshu.png")
	end
        ...
        function(sender,event)
            ...
            if event == TOUCH_EVENT_ENDED then
                ...
                if bm.round == bm.round_total then
					...
					--再来一局，开始新游戏
					bm.playAgainFlag = true
					require("majiang.majiangServer"):LoginGame(USER_INFO["GroupLevel"])
    ...
    for k,v in pairs(pack.content) do
    	...
    	item_ly:setPosition(cc.p(21, (content_ly:getContentSize().height - 23) - (88 * (k - 1) ) ))
    	...
    	if mingGangNum > 0 then
	    	...
	    	for k,v in pairs(mingGangArray) do
	    		...
	    		for i = 1, 4 do
		    		...
		    		card_ly:setPosition(cc.p(position_x, showCard_ly:getContentSize().height))
	...
	elseif isWin == 2 then
		--结果标签显示输的样式
		result_bg_iv:loadTexture("hall/group/create/mahjong_lose_bt.png")
		result_iv:loadTexture("majiang/common/mahjong_lose.png")
	elseif isWin == 0 then
		--没有赢输，则结果标签改成流局的样式
...
function MajiangroomHandle:showGroupResult(pack)
	...
	--获取总结算界面
    local s = cc.CSLoader:createNode("majiang/common/GameResult.csb")
    s:setName("GameResult")
    SCENENOW["scene"]:addChild(s,9998)

    local jzdb_sp = cc.Sprite:create("majiang/image/text_jinzhidubo.png")
    jzdb_sp:setAnchorPoint(0.5, 0.5)
    jzdb_sp:setPosition(cc.p(480, 520))
    s:addChild(jzdb_sp)
    ...
    if #playerlist > 0 then
    	...
    	for k,v in pairs(playerlist) do
	    	...
	    	--定义结果分项
	    	local result_Item = cc.CSLoader:createNode("majiang/common/GameResult_Item.csb")
	    	result_Item:setLocalZOrder(zOrder)
	    	zOrder = zOrder - 1
	    	content_sv:addChild(result_Item)

	    	local item_ly = result_Item:getChildByName("item_ly")
	    	item_ly:setPosition(cc.p(219 * (k - 1), content_sv:getContentSize().height))
...
function MajiangroomHandle:SVR_GROUP_TIME(pack)
	...
	SCENENOW["scene"]:runAction(cc.Sequence:create(cc.DelayTime:create(2),cc.CallFunc:create(function ( )
		-- body
			SCENENOW["scene"]:set_left_card_num_visible(true);
	end)))
 ...
 --    	--记录服务器返回的组局时间
	...
	-- local m = true
	-- while m do
	-- 	if bm.isGroup and SCENENOW["name"]  == "majiang.scenes.MajiangroomScenes" and SCENENOW["scene"]._scene then
	-- 		m = false
	-- 		require("majiang.gameScene"):showTimer(pack)
	-- 	end
	-- end
...
function MajiangroomHandle:SVR_GROUP_BILLBOARD(pack)
    ...
    local game_result = SCENENOW["scene"]:getChildByName("game_result")
    if game_result then
    	local disband_ly = game_result:getChildByName("disband_ly")
    	if disband_ly then
    		disband_ly:setVisible(false)
    	end
    end
...
function MajiangroomHandle:G2H_CMD_REFRESH_DISSOLVE_LIST(pack)
	...
	--申请解散者信息
	local applyer_info = {}
	print(type(applyId),11111,type(tonumber(UID)), applyId == tonumber(UID))
	if applyId == tonumber(UID) then
		...
		if bm.Room ~= nil then
			if bm.Room.UserInfo ~= nil then
				...
				dump(bm.Room.UserInfo,"G2H_CMD_REFRESH_DISSOLVE_LIST")
...
function MajiangroomHandle:CHAT_MSG( pack )
	...
	local index = self:getIndex(pack.uid or tonumber(UID))
...
function MajiangroomHandle:s2c_JOIN_MATCH_RETURN(pack)
	print("===recieve====0x211====================")
	dump(pack)
	print("tonumber(UID)------------------",tonumber(UID))
...
function MajiangroomHandle:SVR_ENTER_PRIVATE_ROOM( pack )

  require("hall.GameTips"):showTips("系统错误", "tohall", 3, "房间不存在")

end

function MajiangroomHandle:BROADCAST_USER_IP(pack)
	-- local x = 0
	...
	-- local is_alert = true
	-- print("IP广播",bm.round,bm.Room.isStart,x)

	-- if bm.round and tonumber(bm.round) > 1 then
	-- 	is_alert = false
	-- else
	-- 	if bm.round and bm.round == 1 and bm.Room.isStart == 1 then
	-- 		is_alert = false
	-- 	end
	-- end
	-- print()

	-- if is_alert and x == 4 then
	-- 	require("hall.GameTips"):showIPAlert(pack["playeripdata"])
	-- end
...
--服务器返回组局收到的信息
function MajiangroomHandle:SERVER_CMD_FORWARD_MESSAGE( pack )
	dump(pack,"-------距离数据---------")   
...
function MajiangroomHandle:SERVER_CMD_MESSAGE( pack )
