...
function MajiangroomServer:quickRoom()
    print("----------bm.palying = true------------",bm.palying) 
    if bm.palying and bm.palying == true then 
        local gametips = require("hall.GameTips")
        gametips:showTips( "游戏已经开始，不能退出房间")
        return
    end
...
function MajiangroomServer:requestHandle(handle,value)
    ...
    dump(handle, "-----麻将操作handle-----")
    dump(value, "-----麻将操作value-----")
...
--聊天相关
function MajiangroomServer:CLI_MSG_FACE(id)
