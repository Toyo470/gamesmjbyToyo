...
--    _G.runScene:retain()
	...
	--获取麻将游戏界面
	local view = cc.CSLoader:createNode("majiang/scens/room.csb"):addTo(self)
	self._scene = view
	...
	local group_count = self._scene:getChildByName("group_count")
	group_count:setVisible(false)
	...
	function changeFnt( parent )
		local txt_score = parent:getChildByName("txt_score")
		local Text_2 = parent:getChildByName("Text_2")
	    if txt_score then
	        local lable = ccui.TextBMFont:create("", "majiang/image/font/num.fnt")
	        lable:setName("txt_score")
	        lable:setScale(0.6)
	        lable:addTo(parent)
	        lable:setPosition(txt_score:getPosition())
	        txt_score:removeSelf()
	    end
	    if Text_2 then
	        local lable = ccui.TextBMFont:create("", "majiang/image/font/num.fnt")
	        lable:setName("Text_2")
	        lable:setScale(0.6)
	        lable:addTo(parent)
	        lable:setPosition(Text_2:getPosition())
	        Text_2:removeSelf()
	    end
	end
	changeFnt(Panel_me)
	changeFnt(Panel_right)
	changeFnt(Panel_left)
	changeFnt(Panel_top)
    ...
    invite_ly:addTouchEventListener(
        function(sender,event)
            ...
            if event == TOUCH_EVENT_ENDED then
            	...
            	require("hall.common.ShareLayer"):showShareLayer("血战到底，房号：" .. USER_INFO["invote_code"], "https://a.mlinks.cc/AK3b", "url", share_content)
    ...
    local sendHandle = require("majiang.scenes.MajiangroomServer")
	...
	btn_huan_ok:onClick(function()
			...
			require("majiang.scenes.MajiangroomServer"):send_Huan_card(tb)
			btn_huan_ok:hide()
			btn_huan_Canle:hide();
...
function MajiangroomScenes:disbandGroup()
	...
	require("hall.gameSettings"):disbandGroup()
...
function MajiangroomScenes:set_ower_info(icon_url,nick_str,gold_num_str,sex_num)
	...
	require("hall.GameCommon"):getUserHead(icon_url, tonumber(UID), sex_num, head_im, 69, false)

    --用户性别
	local sex_spr = panel_me:getChildByName("sex")
	sex_spr:setVisible(false)
	if sex_num == 1 then
		sex_spr:setTexture("majiang/image/nan.png")
	else
		sex_spr:setTexture("majiang/image/nv.png")
	end

    local x = 0
    local y = 0
    x = panel_me:getPositionX()
    y = panel_me:getPositionY() - panel_me:getContentSize().height/2
    x = x + head_im:getPositionX() + head_im:getContentSize().width/2
    y = y + head_im:getPositionY()
...
function MajiangroomScenes:show_otherplayer_info(other_index,icon_url,nick_name,user_gold,sex_num,uid)
    ...
    if panel ~= nil then
	    ...
	    local x = 0
	    local y = 0
	    x = panel:getPositionX()
	    y = panel:getPositionY() - panel:getContentSize().height/2
	    x = x + head_im:getPositionX() + head_im:getContentSize().width/2
	    y = y + head_im:getPositionY()
	    require("hall.view.voicePlayView.voicePlayView"):updateUserPosition(tonumber(uid), cc.p(x, y))
    end
...
function MajiangroomScenes:set_left_card_num_visible(falg,num_str)
	...
	if falg == true then
		...
		if bm.round and bm.round_total then
			group_count:setString("第" .. tostring(bm.round) .. "/" .. tostring(bm.round_total) .. "局")
		end
		
		if num_str then
			left_card_num:setString(num_str)
		end
	end

	left_card_num:setVisible(falg)
	group_count:setVisible(falg)
...
function MajiangroomScenes:set_left_card_num(num)
	...
	-- 	self:set_left_card_num_visible(true,"剩余: "..tostring(num))
	-- else
	-- 	-- left_num:setString("剩余牌数: "..tostring(num))
	-- 	left_num:setString("剩余: "..tostring(num))
	-- end
...
function MajiangroomScenes:set_over_layout(flag)
	...
	if flag == true then
		local view            = cc.CSLoader:createNode("majiang/scens/layer_over.csb"):addTo(self)
	...
	bm.SchedulerPool:loopCall(function()
		...
		if bShowLight == 1 then
			...
			if bm.User.Pque == 0 then
				wang:getChildByName("Image_6"):loadTexture("majiang/image/mahjong_wan_bt_s.png")
			elseif bm.User.Pque == 1 then
				tong:getChildByName("Image_6"):loadTexture("majiang/image/mahjong_tong_bt_s.png")
			elseif bm.User.Pque == 2 then
				tiao:getChildByName("Image_6"):loadTexture("majiang/image/mahjong_suo_bt_s.png")
			end
			
		else
			
			if bm.User.Pque == 0 then
				wang:getChildByName("Image_6"):loadTexture("majiang/image/mahjong_wan_bt.png")
			elseif bm.User.Pque == 1 then
				tong:getChildByName("Image_6"):loadTexture("majiang/image/mahjong_tong_bt.png")
			elseif bm.User.Pque == 2 then
				tiao:getChildByName("Image_6"):loadTexture("majiang/image/mahjong_suo_bt.png")
			end
...
function MajiangroomScenes:show_timer_index(index)
	...
	--index为当前出牌的位置--获取玩家显示的位置号 0自己，1左边玩家，2对家，3右边玩家
	dump(index, "-----当前出牌位置-----")
	...
	Sprite_3:setTexture("majiang/image/timer_nan.png")
	Sprite_1:setTexture("majiang/image/timer_bei.png")
	...
	--显示当前出牌方向
	local new_index = index - bm.zuowei_zhuan
	if new_index < 0 then
		new_index = new_index + 4
	end
	dump("Sprite_"..tostring(new_index), "-----当前显示位置-----")
	direction_ly:getChildByName("Sprite_"..tostring(new_index)):setVisible(true)
...
function MajiangroomScenes:HttpMatchLoadBattles()
    -- body
    require("majiang.MatchSetting"):setRankInfo(require("majiang.ddzSettings"):getMatchLevelRankInfo(USER_INFO["gameLevel"]))
    if require("majiang.MatchSetting"):getMatchState() == 1 then
...
function MajiangroomScenes:setBtns(flag)
	invite_ly:setVisible(flag)
	disband_ly:setVisible(flag)
end
