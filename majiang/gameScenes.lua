...
majiangGameMode = 0
local datamanager = require("majiang.datamanager")
...
function MajianghallScenes:ctor()
	local view            = cc.CSLoader:createNode("majiang/scens/SelectMode.csb"):addTo(self)
	self._scene           = view
    ...
    if head then
        require("hall.GameCommon"):getUserHead(USER_INFO["icon_url"],tonumber(UID),USER_INFO["sex"],head,81,true)
    end
	...
	local function touchButtonEvent(sender,event)
        ...
        if event == TOUCH_EVENT_BEGAN then
            require("hall.GameCommon"):playEffectSound("majiang/music/Audio_Button_Click.mp3")
            sender:setScale(0.9)
        end
        if event == 3 then
            sender:setScale(1)
	...
	--快速进入自由场
	require("majiang.majiangServer"):LoginGame(USER_INFO["gameLevel"])
