...
function mjSetting:show_help_layout(Parent,mode)
    ...
    --获取帮助界面视图资源
    local help_layout = cc.CSLoader:createNode("majiang/scens/LayerHelp.csb")
...
function mjSetting:show_setting_layout(Parent)
    ...
    --获取设置界面视图资源
    local help_layout = cc.CSLoader:createNode("majiang/scens/LayerSetting.csb")
    Parent:addChild(help_layout)
    help_layout:setLocalZOrder(10000)
    ...
    if SHOCK_ON then
        str = "majiang/majiang/Settings/btn_on.png"
    else
        str = "majiang/majiang/Settings/btn_off.png"
    end
...
function show_particle_effect(parent_scence,positonX,positonY,sprite_path)
    ...
    backbg:setScale(0.1)
    backbg:setPosition(cc.p(positonX,positonY))
    parent_scence:addChild(backbg,1600)
    backbg:runAction(action_sequence)

    local _emitter =cc.ParticleSystemQuad:create("majiang/image/Plist/NewParticle_1.plist")
    ...
    _emitter:setPosition(cc.p(positonX,positonY))

    local _emitter =cc.ParticleSystemQuad:create("majiang/image/Plist/NewParticle_2.plist")
    local batch =cc.ParticleBatchNode:createWithTexture(_emitter:getTexture())
    batch:addChild(_emitter)
    ...
    if sprite_path ~= nil then
        ...
        special_spr:setPosition(cc.p(positonX,positonY))
...
function show_guan_effect(parent_scence,positonX,positonY)
    -- body
    show_particle_effect(parent_scence,positonX,positonY,"majiang/image/effect_gang.png")
...
function show_peng_effect(parent_scence,positonX,positonY)
    -- body
    show_particle_effect(parent_scence,positonX,positonY,"majiang/image/effect_peng.png")
...
function show_ting_effect(parent_scence,positonX,positonY)
    -- body
    show_particle_effect(parent_scence,positonX,positonY,"majiang/image/quan_ting.png")
...
function show_zimo_effect(parent_scence,positonX,positonY,hu_type)
    ...
    if hu_type == 2 then
        local sprite_path = "majiang/image/effectzimo.png"

        local special_spr = display.newSprite(sprite_path)
        special_spr:setPosition(cc.p(positonX,positonY + 25 ))
    ...
    if hu_type == 1 then
        local sprite_path = "majiang/image/effect_pinghu.png"
        local special_spr = display.newSprite(sprite_path)
        special_spr:setPosition(cc.p(positonX,positonY + 25 ))
        parent_scence:addChild(special_spr,1601)
...
function show_win_layout( scene )
    ...
    local layout = cc.CSLoader:createNode("majiang/scens/Layerwin.csb"):addTo(scene)
...
function show_lose_layout(scene)
    ...
    local layout = cc.CSLoader:createNode("majiang/scens/Layerlose.csb"):addTo(scene)
    layout:setName("layout_lose")
    layout:setLocalZOrder(10000)
