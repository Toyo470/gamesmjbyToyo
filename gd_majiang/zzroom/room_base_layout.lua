...
function RoomBaseLayout:_do_after_init()
	...
	self.room_base_gamemsg:setString("广东麻将：".. tostring(gameConfig))
...
function RoomBaseLayout:set_gamemsg( game_msg )
	-- body
	self.room_base_gamemsg:setString("广东麻将："..tostring(game_msg))
end
...
function RoomBaseLayout:set_room_base_video_ex(flag)
	print("set_room_base_video_ex", tostring(isiOSVerify), tostring(flag))
    if isiOSVerify then
        if self.room_base_video then
        	self.room_base_video:setVisible(false)
        end
    else
