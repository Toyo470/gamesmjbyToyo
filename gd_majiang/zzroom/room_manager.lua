...
function RoomManager:remove_hand_card(index,card,mount)
  ...
  print("remove_hand_card", tostring(index), tostring(card), tostring(mount))
  ...
  dump(mytable, "remove_hand_card", nesting)
  return true
end
...
function RoomManager:deal_gang( index,cards_value,gang_type,gang_result)
  ...
  if index ==  0 then
    ...
    if gang_result == 0x400 then --补杆
        self:remove_hand_card(index,0,1)
    elseif gang_result == 0x200 then--暗杠
...
function RoomManager:set_player_ip( uid,ip )
  ...
  dump(self.uid_ip_dict, "set_player_ip", nesting)
...
function RoomManager:after_set_uid_ip()
  -- body
  mrequire("tips")
  dump(self.uid_name_dict, "after_set_uid_ip", nesting)
  if table.nums(self.uid_name_dict) >= 3 then
    dump(self.uid_ip_dict,"self.uid_ip_dict")
    for _ip,uid_tbl in pairs(self.uid_ip_dict) do
      uid_tbl =uid_tbl or {}
