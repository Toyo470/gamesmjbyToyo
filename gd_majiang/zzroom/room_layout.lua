...
function ZZRoom_Layout:click_room_btn_exit_event()
	-- local gd_Handle = require("gd_majiang.gd_Handle")
	-- local tbl = {}
 --    tbl["card"]    = 38
 --     tbl["handle"]    = 0x010
 ...
 --    gd_Handle:SVR_OWN_CATCH_BROADCAST(tbl) --测代码
  ...
  --  layout_object:draw_zhua_card(3) 
	...
	print("click_room_btn_exit_event", tostring(bm.Room.isStart))
	-- if bm.Room and bm.Room.isStart and bm.Room.isStart == 1 then
	if require("hall.gameSettings"):getGroupState() == 1 then
		tips.show_tips("go_hall_title","exit_game_content",handler(self,self.cancal),handler(self,self.cancal))
	else
		tips.show_tips("go_hall_title","go_hall_content",handler(self,self.go_hall),handler(self,self.cancal))
	end
