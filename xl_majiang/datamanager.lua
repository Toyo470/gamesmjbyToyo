   ...
   -- dump(data)
    ...
    for _,room in pairs(data.netData.data.roomList) do 
    	if room.level == level then
	        ...
	        print("level",level)
	        dump(tbRank,"tbRank")
        	require("xl_majiang.ddzSettings"):addMatchRank(level,tbRank)
 			local rankinfo = require("xl_majiang.ddzSettings"):getMatchLevelRankInfo(level)
 			require("xl_majiang.MatchSetting"):setRankInfo(rankinfo)
    		break;
    	end
    end
