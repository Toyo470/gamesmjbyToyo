...
function SocketTCP:isSupportIpv6(_domain)
...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	if self:isSupportIpv6(self.host) then
		--todo
...
function SocketTCP:close( ... )
	--printError("系统发过来的消息断开了%s.close")
	print("scoket close zengtao")
	self.tcp:close();
...
function SocketTCP:_onConnected()
	...
	local __tick = function()
		while true do
			...
			local __body, __status, __partial ,__arg3,__arg4= self.tcp:receive("*a")	-- read the package body
			--print("body:", __body, "__status:", __status, "__partial:", __partial,__arg3,__arg4)
