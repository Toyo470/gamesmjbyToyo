...
function RoundEndingLayer:show(data, isEnd)
	...
	self._scene = cc.CSLoader:createNode("kawuxing/roundEndingLayer.csb")
	...
	for i=1,table.getn(players) do
		...
		if KWX_ZHUANG_UID == players[i].uid then
			--todo
			isZhuang = true
		end
...
function RoundEndingLayer:showPlayerPlane(player, index, isZhuang, huCard, endType)
	...
	local nickname = KWX_USERINFO_TABLE[seatId .. ""].nick
...
function RoundEndingLayer:showEndingInfo(player, index)
	...
	if isChajiao and player.huTypeCount == 0 then
		--todo
		endingInfo = endingInfo .. "查叫 "
	end

	for i=1,player.huTypeCount do
		endingInfo = endingInfo .. KWX_HU_TYPE_TABLE[player.huTypes[i] + 1] .. " "
	end
...
function RoundEndingLayer:showTitle(titleType)
	...
	if titleType == TITLE_TYPE_WIN then
		--todo
		title_bg_img:loadTexture("kawuxing/image/red_button_p.png")
		title_img:loadTexture("kawuxing/image/mahjong_win.png")
	elseif titleType == TITLE_TYPE_LOSE then
		title_bg_img:loadTexture("kawuxing/image/mahjong_lose_bt.png")
		title_img:loadTexture("kawuxing/image/mahjong_lose.png")
	elseif titleType == TITLE_TYPE_LIUJU then
		title_bg_img:loadTexture("kawuxing/image/grey_button.png")
		title_img:loadTexture("kawuxing/image/text_liuju.png")
	else
		--todo
		title_bg_img:loadTexture("kawuxing/image/mahjong_lose_bt.png")
		title_img:loadTexture("kawuxing/image/pingju.png")
	end
end
...
function RoundEndingLayer:showGameRemark(timeStr)
	...
	if USER_INFO["gameConfig"] then
		--todo
		bird_lb:setString("卡五星：" .. USER_INFO["gameConfig"])
	else
