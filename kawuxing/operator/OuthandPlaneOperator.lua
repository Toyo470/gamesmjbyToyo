...
function OuthandPlaneOperator:playCard(playerType, cardValue, plane)
	...
	if playerType == CARD_PLAYERTYPE_MY then
		...
		local function card_callback()  
		   ...
		   KWX_CARD_POINTER:setPosition(cc.p(worldPoint.x + cardNode:getSize().width / 2, worldPoint.y + cardNode:getSize().height + KWX_CARD_POINTER:getSize().height / 4))
		   KWX_CARD_POINTER:setVisible(true)
