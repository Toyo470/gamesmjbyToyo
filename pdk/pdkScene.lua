cc.FileUtils:getInstance():addSearchPath("pdk/")
...
local pdkScene = class("pdkScene", function()
    return display.newScene("pdkScene")
end)
...
--    gChangeOrientation(1)
    local s = cc.CSLoader:createNode("pdk/csb/SelectMode.csb"):addTo(self)
    self._scene = s
...
--绑定按钮声音
function pdkScene:addBtAudio(a,b,c,d,e)
...
function pdkScene:HttpLoadGame(modeList)
