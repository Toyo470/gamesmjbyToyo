...
function PlayVideo:setStatue(flag)
    ...
    if btn_live_video then
        if isShowVideo then
            isStartLive = 1
            btn_live_video:loadTextureNormal("pdk/images/live_bt_n.png")
            -- btn_live_video:setColor(cc.c3b(255,255,255))
            btn_live_video:setEnabled(true)
        else
            isStartLive = 0
            btn_live_video:loadTextureNormal("pdk/images/live_bt_p.png")
            -- btn_live_video:setColor(cc.c3b(255,255,255))
            btn_live_video:setEnabled(true)
        end
    end
    ...
    if (cc.PLATFORM_OS_ANDROID == targetPlatform) then
        ...
        if tbData["returnCode"] ~= "0" then
            ...
            if tbData["command"] == "startLive" then--开直播
                ...
                --广播直播地址
                require("pdk.pdkServer"):CLI_SEND_LIVE_ADDRESS(json.encode(tbdata),1)

                isStartLive = 1
                cc.UserDefault:getInstance():setStringForKey("getLiveData","")
            elseif tbData["command"] == "stopLive" then--关直播
                require("pdk.pdkServer"):CLI_SEND_LIVE_ADDRESS("",0)
                cc.UserDefault:getInstance():setStringForKey("getLiveData","")
                isGetLiveAddress = false
                self:setStatue(false)
                isStartLive = 0
        ...
        local function callbackVideo(param)
            ...
            if tbData["returnCode"] ~= "0" then
                ...
                if tbData["command"] == "startLive" then--开直播
                    ...
                    -- --广播直播地址
                    -- xpcall(require("pdk.pdkServer"):CLI_SEND_LIVE_ADDRESS(json.encode(tbdata),1),function()
                    --         printError("出现的错误了")
                    --     end)
                elseif tbData["command"] == "stopLive" then--关直播
                    require("pdk.pdkServer"):CLI_SEND_LIVE_ADDRESS("",0)
