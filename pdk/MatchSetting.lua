...
function MatchSetting:showMatchSignup(flag,signcount,totalcount,entryFee,code,matchlevel)
    ...
    require("pdk.pdkHttpNet"):requestIncentive(matchlevel)
        ...
        local function touchEvent(sender, event)
            ...
            if event == TOUCH_EVENT_ENDED then
                ...
                if sender == btnSignup then
                    ...
                    pdkServer:CLI_SEND_JOIN_MATCH(gameLevel)
                    self:setRankInfo(require("pdk.pdkSettings"):getMatchLevelRankInfo(gameLevel))
                ...
                if sender == btnMatchLogout then
                    ...
                    require("pdk.pdkServer"):CLI_SEND_LOGOUT_MATCH(gameLevel)
                    btnMatchLogout:setEnabled(false)
                    btnMatchLogout:setVisible(false)
                end
...
function MatchSetting:createRanking(round)
    ...
    for i, v in ipairs(matchInfoList) do
        --横杠
        if i > 1 then
            local spLine = display.newSprite("pdk/match/promotion_mingci_box01.png")
            if spLine then
        ...
        local strFont = ""
        if i == round then
            str = "pdk/match/promotion_mingci_p.png"
            strFont = "pdk/match/promotion_mingci_green.fnt"
        else
            str = "pdk/match/promotion_mingci_n.png"
            strFont = "pdk/match/promotion_mingci.fnt"
        end
        local spBack = display.newSprite("pdk/match/promotion_mingci_box02.png")
...
function MatchSetting:showMatchResult()
    ...
    local match_result = cc.CSLoader:createNode("pdk/match/MatchResult.csb"):addTo(SCENENOW["scene"])
    ...
    if btnExit then
        btnExit:addTouchEventListener(function(event,type)
            if(type == 2) then
                require("hall.GameCommon"):playEffectSound("hall/Audio_Button_Click.mp3")
                --退出游戏
                require("pdk.pdkServer"):CLI_SEND_LOGOUT_MATCH(require("pdk.pdkSettings"):getMatchId())
                --避免重复点击
