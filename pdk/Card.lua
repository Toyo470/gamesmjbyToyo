...
function  Card:getCard(value,kind,isLandlord,bShow)
    ...
    --底
    local spBack = cc.Scale9Sprite:create("pdk/Game/cards/small_card_bg.png")
	...
	-- spBack:setColor(ccc3(245, 245, 245))
...
function Card:getLogo(flag)
    ...
    local spBack = display.newSprite("pdk/images/lord_bar.png")
    if spBack then
        local sp = display.newSprite("pdk/images/lord_bar_text_dizhu.png")
        spBack:addChild(sp)
...
function Card:getSmallCard(value,kind)
    ...
    local scale = 0.75
    --底
    local spBack = display.newSprite("pdk/Game/cards/small_card_bg.png")
    spBack:setAnchorPoint(cc.p(0.5,0.5))
    --字
    local path = "#"..tbKindValue[tonumber(kind)]..tostring(value)..".png"
    local spUpValue = display.newSprite(path)
