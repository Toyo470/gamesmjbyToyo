...
function InhandPlaneOperator:showCards(playerType, plane, cardDatas, cardDatas_state)
	plane:removeAllChildren()
	...
	if playerType == CARD_PLAYERTYPE_MY then
		...
		local function onTouchBegan(touch, event)
		    if TDHMJ_CHUPAI == 1 then
				...
				if cardTemp and cardTemp:getTag() ~= NEW_CARD_TAG then
					...
					TDHMJ_CONTROLLER:hideSameCard()
					TDHMJ_CONTROLLER:hideTingHuPlane()
					...
					TDHMJ_CONTROLLER:showSameCard(cardTemp.m_value)
		...
		local function onTouchEnded(touch, event)
		   	if cardTemp then
		   		...
		   		if offsetY > plane:getSize().height then
		   			--超出范围，出牌
		   			TDHMJ_CONTROLLER:playCard(cardTemp.m_value)
		...
		for i=1,table.getn(cardDatas) do
			...
			if TDHMJ_ting == 1 then
				local card = Card:new(playerType, CARD_TYPE_INHAND, CARD_DISPLAY_TYPE_OPPOSIVE, data,true)
...
function InhandPlaneOperator:showCardsForAll(playerType, plane, cardDatas, anke)
	...
	elseif playerType == CARD_PLAYERTYPE_LEFT then
		...
		for i=1,count do
			...
			if cardData ~= -1 then
				--todo
				card = Card:new(playerType, CARD_TYPE_LEFTHAND, CARD_DISPLAY_TYPE_SHOW, cardData,false)
			else
				card = Card:new(playerType, CARD_TYPE_LEFTHAND, CARD_DISPLAY_TYPE_HIDE, cardData,false)
			end
...
function InhandPlaneOperator:getNewCard(playerType, plane, value)
	if playerType == CARD_PLAYERTYPE_MY then
		...
		card:addTouchEventListener(function(sender, event)
			    ...
			    if event == TOUCH_EVENT_BEGAN then
			    	...
			    	TDHMJ_CONTROLLER:hideSameCard()   
                	TDHMJ_CONTROLLER:showSameCard(value)

			    end

				if event == TOUCH_EVENT_ENDED then
					--todo
					if sender:getTag() == NEW_CARD_TAG and TDHMJ_CHUPAI == 1 then
						--todo
	...
	elseif playerType == CARD_PLAYERTYPE_LEFT then
		local card = Card:new(playerType, CARD_TYPE_INHAND, CARD_DISPLAY_TYPE_OPPOSIVE, value,false)
		card:setTag(NEW_CARD_TAG)
...
-------听牌返回
              ...
              for k,v in pairs(tingSeq) do
                   if cardTemp.m_value == v.card then
                      isTing = true
                   break
                   end
              end
...
-----
              TDHMJ_CONTROLLER:hideTingHuPlane()
              	   ...
              	   if tingSeq~=nil and  next(tingSeq)~=nil  then  --听队列存在时才遍历
					  for k,v in pairs(tingSeq) do
						if v.card == cardTemp.m_value then
							...
							for i,x in pairs(v.tingHuCards)	do	
							dump(x.huCards,"显示听得牌2")	
								TDHMJ_CONTROLLER:showTingHuPlane(x.huCards)
							end
						break
						end
					  end
        ...
        -- end
        -- local function onTouchMoved1(touch, event)
        -- end
        -- local function onTouchEnded1(touch, event)
        -- end

        -- local listener3 = cc.EventListenerTouchOneByOne:create()
        -- listener3:setSwallowTouches(true)
        -- listener3:registerScriptHandler(onTouchBegan1,cc.Handler.EVENT_TOUCH_BEGAN )
        -- listener3:registerScriptHandler(onTouchMoved1,cc.Handler.EVENT_TOUCH_MOVED )
        -- listener3:registerScriptHandler(onTouchEnded1,cc.Handler.EVENT_TOUCH_ENDED )

        local eventDispatcher = SCENENOW["scene"]:getEventDispatcher()
            
            -- local cardDatas_state=cardDatas_state
  for i=1,table.getn(cardDatas) do
      local data = cardDatas[i]
      ...
      local isnotTing=not isTing
                -- local flag = cardUtils:judge(cardDatas_state[i])
          local card = Card:new(playerType, CARD_TYPE_INHAND, CARD_DISPLAY_TYPE_OPPOSIVE, data,isnotTing)
       ...
       if isTing==true then
          eventDispatcher:addEventListenerWithSceneGraphPriority(listener2:clone(),card)
          card:setTouchEnabled(false)
       else
       	  card:setTouchEnabled(false)
          card:setColor(cc.c3b(150, 150, 150))
