...
function CenterPlaneOperator:changeTurn(playerType, plane)
	local file = "tdh/image/mahjong_turn_" .. playerType .. ".png"
...
function CenterPlaneOperator:beginPlayCard(playerType, plane)
	...
	local showTimerAc = cc.CallFunc:create(function()
			...
			if timer_value == 0 then
				--todo
				timer_lb:stopAllActions()
				require("tdh.operator.GamePlaneOperator"):showNetworkImg(playerType,true)
			end
		end)
...
function CenterPlaneOperator:rotateTimer(zhuang_index,plane)
	...
	-- 重置标志器指向
	location_img = plane:getChildByName("location_img")
	location_img:setRotation(0)
    
    dump(zhuang_index,"庄家位置")
	--进行旋转（把东指向庄）
	if zhuang_index == CARD_PLAYERTYPE_MY then
		location_img:setRotation(0)
	elseif zhuang_index == CARD_PLAYERTYPE_LEFT then
		location_img:setRotation(90)
	elseif zhuang_index == CARD_PLAYERTYPE_TOP then
		location_img:setRotation(180)
	elseif zhuang_index ==  CARD_PLAYERTYPE_RIGHT then
		location_img:setRotation(270)
	end
